...
public class SmallFundIntentListPro {
	/**
	 * 贷款贷出
	 */
	...
	public static List<SlFundIntent> getFundIntentDefaultList(String calcutype,
			...
			if(!isStartDatePay.equals("1")){//按实际放款日对日还款
				if(isInterestByOneTime==0){//不一次性首付利息
		    	   for (int i = 1; i <=payintentPeriod; i++) {
		    		   ...
		    		   sf1 =calculslfundintent(SmallFundIntentListPro.ProjectLoadAccrual,            // 资金类型
		    				   intentDate,  //计划日期
		    		   ...
		    		   sf =calculslfundintent(SmallFundIntentListPro.ProjectLoadRoot,            // 资金类型
					   ...
					   sf2 = calculslfundintent(SmallFundIntentListPro.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,managementConsultingOfRate),i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay ,null,accrualType        // 收入金额
								);
					 /*  sf3 = calculslfundintent(SmallFundIntentListPro.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,financeServiceOfRate),i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);*/
	    		   ...
	    		   if(isPreposePayAccrual == 1){//前置付息
	    			 ...
	    			 if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
					   ...
					   sf2 = calculslfundintent(SmallFundIntentListPro.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(payintentPeriod)),1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					  /* sf3 = calculslfundintent(SmallFundIntentListPro.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(payintentPeriod)),1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay ,null,accrualType        // 收入金额
								);*/
					   if(sf2.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
					    list.add(sf2);
					   }
				   ...
				   for (int i = 1; i <=payintentPeriod; i++) {
		    		   if(isPreposePayAccrual ==0 && i==payintentPeriod){//前置付息
						   ...
						   sf2 = calculslfundintent(SmallFundIntentListPro.ProjectManagementConsulting,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
									SmallFundIntentListPro.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(payintentPeriod)),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						  /* sf3 = calculslfundintent(SmallFundIntentListPro.ProjectFinanceService,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
									SmallFundIntentListPro.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(payintentPeriod)),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType         // 收入金额
									);*/
						   if(sf2.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
						    list.add(sf2);
						   }
				...
				if(isInterestByOneTime==0){//不一次性首付利息
		    	   for (int i = 1; i <=payintentPeriod; i++) {
					   ...
					   sf2 = calculslfundintent(SmallFundIntentListPro.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(30)),i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					  /* sf3 = calculslfundintent(SmallFundIntentListPro.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(30)),i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);*/
					   if(sf2.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
					    list.add(sf2);
					   }
	    		   ...
	    		   if(isPreposePayAccrual == 1){//前置付息
	    			 ...
	    			 if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
					   ...
					   sf2 = calculslfundintent(SmallFundIntentListPro.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)),1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					  /* sf3 = calculslfundintent(SmallFundIntentListPro.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)),1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);*/
					   if(sf2.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
					    list.add(sf2);
					   }
				   ...
				   for (int i = 1; i <=payintentPeriod; i++) {
		    		   ...
		    		   if(isPreposePayAccrual == 0 && i==payintentPeriod){
						   ...
						   sf2 = calculslfundintent(SmallFundIntentListPro.ProjectManagementConsulting,            // 资金类型
								   DateUtil.addMonthsToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), payintentPeriod),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
									SmallFundIntentListPro.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						 /*  sf3 = calculslfundintent(SmallFundIntentListPro.ProjectFinanceService,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.addMonthsToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), payintentPeriod),-1),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
									SmallFundIntentListPro.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);*/
						   if(sf2.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
						    list.add(sf2);
						   }
				...
				if(isInterestByOneTime==0){//不一次性首付利息
		    	   for (int i = 1; i <=payintentPeriod; i++) {
					   ...
					   sf2 = calculslfundintent(SmallFundIntentListPro.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(3)),i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					 /*  sf3 = calculslfundintent(SmallFundIntentListPro.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(3)),i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);*/
					   if(sf2.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
					    list.add(sf2);
					   }
	    		   ...
	    		   if(isPreposePayAccrual == 1){//前置付息
	    			 ...
	    			 list.add(sf1);
					   ...
					   sf2 = calculslfundintent(SmallFundIntentListPro.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)).multiply(new BigDecimal(3)),1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay ,null,accrualType         // 收入金额
								);
					   /*sf3 = calculslfundintent(SmallFundIntentListPro.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)).multiply(new BigDecimal(3)),1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay ,null,accrualType         // 收入金额
								);*/
					   if(sf2.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
					    list.add(sf2);
					   }
				   ...
				   for (int i = 1; i <=payintentPeriod; i++) {
		    		   ...
		    		   if(isPreposePayAccrual == 0 && i==payintentPeriod){
						   ...
						   sf2 = calculslfundintent(SmallFundIntentListPro.ProjectManagementConsulting,            // 资金类型
								   DateUtil.addMonthsToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i*3),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
									SmallFundIntentListPro.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)).multiply(new BigDecimal(3)),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						  /* sf3 = calculslfundintent(SmallFundIntentListPro.ProjectFinanceService,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.addMonthsToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i*3),-1),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
									SmallFundIntentListPro.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)).multiply(new BigDecimal(3)),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType         // 收入金额
									);*/
						   if(sf2.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
						    list.add(sf2);
						   }
				...
				if(isInterestByOneTime==0){//不一次性首付利息
		    	   for (int i = 1; i <=payintentPeriod; i++) {
					   ...
					   sf2 = calculslfundintent(SmallFundIntentListPro.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(dayOfEveryPeriod)),i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   /*sf3 = calculslfundintent(SmallFundIntentListPro.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(dayOfEveryPeriod)),i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);*/
					   if(sf2.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
					    list.add(sf2);
					   }
	    		   ...
	    		   if(isPreposePayAccrual == 1){//前置付息
	    			 ...
	    			 list.add(sf1);
					   ...
					   sf2 = calculslfundintent(SmallFundIntentListPro.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(dayOfEveryPeriod)).multiply(new BigDecimal(payintentPeriod)),1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					 /*  sf3 = calculslfundintent(SmallFundIntentListPro.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(dayOfEveryPeriod)).multiply(new BigDecimal(payintentPeriod)),1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);*/
					   if(sf2.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
					    list.add(sf2);
					   }
				   ...
				   for (int i = 1; i <=payintentPeriod; i++) {
		    		   ...
		    		   if(isPreposePayAccrual ==0 && i==payintentPeriod){//前置付息
						   ...
						   sf2 = calculslfundintent(SmallFundIntentListPro.ProjectManagementConsulting,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i*dayOfEveryPeriod),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
									SmallFundIntentListPro.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(dayOfEveryPeriod)).multiply(new BigDecimal(payintentPeriod)),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						   /*sf3 = calculslfundintent(SmallFundIntentListPro.ProjectFinanceService,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i*dayOfEveryPeriod),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
									SmallFundIntentListPro.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(dayOfEveryPeriod)).multiply(new BigDecimal(payintentPeriod)),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);*/
						   if(sf2.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
						    list.add(sf2);
						   }
	           ...
	           for (int i = 0; i < payintentPeriod; i++) {
	        	   Date intentDate=null;
					 ...
					 sf1 = calculslfundintent(SmallFundIntentListPro.ProjectLoadAccrual,            // 资金类型
							 intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,accrual),i+1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType 	);        // 收入金额
					 if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
					 list.add(sf1);
					 }
					 sf2 = calculslfundintent(SmallFundIntentListPro.ProjectManagementConsulting,            // 资金类型
					    		intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,managementConsultingOfRate),i+1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					 /*sf3 = calculslfundintent(SmallFundIntentListPro.ProjectFinanceService,            // 资金类型
								intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListPro.calculAccrualnew(projectMoney,financeServiceOfRate),i+1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);*/
				...
				if(isPreposePayAccrual==1){
				 ...
				 sf1 = calculslfundintent(SmallFundIntentListPro.ProjectLoadAccrual,            // 资金类型
						 intentDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							SmallFundIntentListPro.calculAccrualnew(projectMoney,accrual).multiply(new BigDecimal(payintentPeriod)),qs,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType 	);        // 收入金额
				 if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
				 list.add(sf1);
				 }
				 sf2 = calculslfundintent(SmallFundIntentListPro.ProjectManagementConsulting,            // 资金类型
				    		intentDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							SmallFundIntentListPro.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(payintentPeriod)),qs,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
							);
				/* sf3 = calculslfundintent(SmallFundIntentListPro.ProjectFinanceService,            // 资金类型
							intentDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							SmallFundIntentListPro.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(payintentPeriod)),qs,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
							);*/
   		 ...
   		 if(isStartDatePay.equals("1")){ //固定在某日
 			...
 			if(isInterestByOneTime==0){
 				for (int i = 1; i < payintentPeriod+1; i++) {
	   			  ...
	   			  BigDecimal actualdays=new BigDecimal(Integer.valueOf(DateUtil.getDaysBetweenDate( sdf.format(startactualdate), sdf.format(endctualdate))+1).toString());
			 ...
			 if(isStartDatePay.equals("1")){ //固定在某日
				...
				// if(payintentPeriod>1){
				 String firstdays=Integer.valueOf(DateUtil.getDaysBetweenDate( date1,sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1)))).toString();
				 	sf1 = calculslfundintent(SmallFundIntentListPro.ProjectLoadAccrual,            // 资金类型
							...
							BigDecimal.valueOf(0),                                                // 支出金额
							SmallFundIntentListPro.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(firstdays)),      // 收入金额
		    	...
		    	//  }
				 ...
				 sf1 = calculslfundintent(SmallFundIntentListPro.ProjectLoadAccrual,            // 资金类型
						 intentDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							SmallFundIntentListPro.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30)),
				 ...
				 sf1 = calculslfundintent(SmallFundIntentListPro.ProjectLoadAccrual,            // 资金类型
						 intDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							SmallFundIntentListPro.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(enddays)),      // 收入金额
				  ...
				  /*  BigDecimal financeServiceOfRate2=financeServiceOfRate.multiply(new BigDecimal(enddays));
					...
					// BigDecimal financeServiceOfRateMoney=new BigDecimal(0);
						 ...
						 String firstdays=Integer.valueOf(DateUtil.getDaysBetweenDate( date1,sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1)))).toString();
						 accrualMoney=accrualMoney.add(SmallFundIntentListPro.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(firstdays)));
						 BigDecimal  managementConsultingOfRate1=managementConsultingOfRate.multiply(new BigDecimal(firstdays));
						 ...
						 for (int i = 2; i <payintentPeriod; i++) {
							 accrualMoney=accrualMoney.add(SmallFundIntentListPro.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30)));
						 ...
						 BigDecimal managementConsultingOfRate2=managementConsultingOfRate.multiply(new BigDecimal(enddays));
						 BigDecimal financeServiceOfRate2=financeServiceOfRate.multiply(new BigDecimal(enddays));
						 accrualMoney=accrualMoney.add(SmallFundIntentListPro.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(enddays)));
		...
		if (accrualType.equals("sameprincipalandInterest") && payaccrualType.equals("monthPay")) {// 月尾
		     BigDecimal periodtimemoney=SmallFundIntentListPro.periodtimemoney(accrual.multiply(new BigDecimal(30)),projectMoney,payintentPeriod).divide(new BigDecimal(1),2,BigDecimal.ROUND_HALF_UP);
		     BigDecimal preperiodsurplus=projectMoney;
		     if(isStartDatePay.equals("1")){ //固定在某日
		    	 if(isInterestByOneTime==0){
			    	 String firstdays=Integer.valueOf(DateUtil.getDaysBetweenDate( date1,sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1)))).toString();
			    	 ...
			    	 sf1 = calculslfundintent(SmallFundIntentListPro.ProjectLoadAccrual,            // 资金类型
			    	  ...
			    	  BigDecimal firstperiodtimemoney=periodtimemoney.add(sf1.getIncomeMoney().subtract(SmallFundIntentListPro.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30))));
					    sf =calculslfundintent(SmallFundIntentListPro.ProjectLoadRoot,            // 资金类型
								DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1),  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
					...
					sf1 = calculslfundintent(SmallFundIntentListPro.ProjectLoadAccrual,            // 资金类型
							...
							BigDecimal.valueOf(0),                                                // 支出金额
							SmallFundIntentListPro.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30)),      // 收入金额
				    ...
				    sf =calculslfundintent(SmallFundIntentListPro.ProjectLoadRoot,            // 资金类型
							DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), i),  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							periodtimemoney.subtract(SmallFundIntentListPro.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30))),        // 收入金额
							i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,String.valueOf(payintentPerioDate),accrualType
				    );
				    if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
						list.add(sf1);
				...
				/*	sf3 = calculslfundintent(SmallFundIntentListPro.ProjectFinanceService,            // 资金类型
					...
					sf1 = calculslfundintent(SmallFundIntentListPro.ProjectLoadAccrual,            // 资金类型
			    	  ...
			    	  BigDecimal endperiodtimemoney=periodtimemoney.subtract((SmallFundIntentListPro.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30))).subtract(sf1.getIncomeMoney()));
					    sf =calculslfundintent(SmallFundIntentListPro.ProjectLoadRoot,            // 资金类型
					    		DateUtil.addMonthsToDate(DateUtil.parseDate(date1), payintentPeriod),  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
		    		...
		    		// BigDecimal financeServiceOfRateMoney=new BigDecimal(0);
			    	 ...
			    	 String firstdays=Integer.valueOf(DateUtil.getDaysBetweenDate( date1,sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1)))).toString();
			    	 accrualMoney=accrualMoney.add(SmallFundIntentListPro.calculAccrualnew(preperiodsurplus1,accrual).multiply(new BigDecimal(firstdays)));
			    	 BigDecimal managementConsultingOfRate1=managementConsultingOfRate.multiply(new BigDecimal(firstdays));
			    	 managementConsultingOfRateMoney=managementConsultingOfRateMoney.add(projectMoney.multiply(managementConsultingOfRate1));
			    	/* BigDecimal financeServiceOfRate1=financeServiceOfRate.multiply(new BigDecimal(firstdays));
			    	 financeServiceOfRateMoney=financeServiceOfRateMoney.add(projectMoney.multiply(financeServiceOfRate1));*/
						...
						preperiodsurplus1=preperiodsurplus1.subtract(periodtimemoney.subtract(SmallFundIntentListPro.calculAccrualnew(preperiodsurplus1,accrual).multiply(new BigDecimal(30))));
						
						
						
					for (int i = 2; i < payintentPeriod; i++) {
						accrualMoney=accrualMoney.add(SmallFundIntentListPro.calculAccrualnew(preperiodsurplus1,accrual).multiply(new BigDecimal(30)));
						managementConsultingOfRateMoney=managementConsultingOfRateMoney.add(projectMoney.multiply(managementConsultingOfRate).multiply(new BigDecimal(30)));
						//financeServiceOfRateMoney=financeServiceOfRateMoney.add(projectMoney.multiply(financeServiceOfRate).multiply(new BigDecimal(30)));
					...
					String enddays=Integer.valueOf(DateUtil.getDaysBetweenDate( sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), payintentPeriod-1)),sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(date1), payintentPeriod)))).toString();
					accrualMoney=accrualMoney.add(SmallFundIntentListPro.calculAccrualnew(preperiodsurplus1,accrual).multiply(new BigDecimal(enddays)));
					BigDecimal managementConsultingOfRate2=managementConsultingOfRate.multiply(new BigDecimal(enddays));
					managementConsultingOfRateMoney=managementConsultingOfRateMoney.add(projectMoney.multiply(managementConsultingOfRate2));
					BigDecimal financeServiceOfRate2=financeServiceOfRate.multiply(new BigDecimal(enddays));
					//financeServiceOfRateMoney=financeServiceOfRateMoney.add(projectMoney.multiply(financeServiceOfRate2));
					...
					if(isPreposePayAccrual==1){
					  ...
					  sf =calculslfundintent(SmallFundIntentListPro.ProjectLoadRoot,            // 资金类型
						...
						preperiodsurplus=preperiodsurplus.subtract(periodtimemoney.subtract(SmallFundIntentListPro.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30))));
						for (int i = 2; i < payintentPeriod; i++) {
					    ...
					    sf =calculslfundintent(SmallFundIntentListPro.ProjectLoadRoot,            // 资金类型
					    		DateUtil.addMonthsToDate(DateUtil.parseDate(date1), payintentPeriod),  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								periodtimemoney.subtract(SmallFundIntentListPro.calculAccrualnew(preperiodsurplus1,accrual).multiply(new BigDecimal(enddays))),        // 收入金额
								payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,String.valueOf(payintentPerioDate),accrualType
					    );
						list.add(sf);
			...
			if(isInterestByOneTime==1){
				...
				if(AppUtil.getInterest().equals("0")){
					if(isPreposePayAccrual==1){
						...
						if(isStartDatePay.equals("1")){
							...
							/*if(intentdateStr.equals(date1)){
								interestEndTime=DateUtil.addDaysToDate(DateUtil.parseDate(intentdateArr[0]+"-"+intentdateArr[1]+"-"+gddate,"yyyy-MM-dd"), -1);
							}
							String interestEndTimeStr=sd.format(interestEndTime);
							if((interestEndTimeStr.substring(0,interestEndTimeStr.lastIndexOf("-"))).equals(date2.substring(0,date2.lastIndexOf("-")))){
								interestEndTime=DateUtil.addDaysToDate(DateUtil.parseDate(date2,"yyyy-MM-dd"), -1);
							}*/
				...
				if(AppUtil.getInterest().equals("0")){
					if(isStartDatePay.equals("1")){
						...
						/*if((Long.valueOf(gddate)<=Long.valueOf(dateArr[2]) && payintentPeriod==1) || (Long.valueOf(gddate)>Long.valueOf(dateArr[2]) && payintentPeriod==0)){
							interestStarTime=DateUtil.parseDate(date1,"yyyy-MM-dd");
						}*/
						if(payintentPeriod==0){
							interestStarTime=DateUtil.parseDate(date1,"yyyy-MM-dd");
					...
					if(isStartDatePay.equals("1")){
						...
						if(intentdateStr.equals(date2)){
							interestStarTime=DateUtil.parseDate(intentdateArr[0]+"-"+(Integer.valueOf(intentdateArr[1])-1)+"-"+String.valueOf(Integer.valueOf(gddate)+1),"yyyy-MM-dd");
						}
					}else{
						if(isPreposePayAccrual==1){
							if(payaccrualType.equals("dayPay")){
