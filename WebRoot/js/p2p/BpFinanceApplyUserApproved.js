 ...
 * @createtime:
 */
BpFinanceApplyUserApproved = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				BpFinanceApplyUserApproved.superclass.constructor.call(this, {
							id : 'BpFinanceApplyUserApproved',
							title : '基础材料通过审核',
							region : 'center',
							layout : 'border',
			...
			initUIComponents : function() {
				...
				this.gridPanel=new HT.GridPanel({
					...
					url : __ctxPath + "/p2p/personListBpFinanceApplyUser.do?state=4",
					fields : [{
									name : 'loanId',
									type : 'int'
								}
				...
				if (rows.length == 0) {
					...
					var loanId=rows[0].data.loanId;
					 Ext.Msg.confirm('信息确认', '是否启动借款审批流程', function(btn) {
				        if (btn == 'yes') {
							Ext.Ajax.request({
								url:__ctxPath + '/p2p/startLoanApprovalFlowBpFinanceApplyUser.do',
