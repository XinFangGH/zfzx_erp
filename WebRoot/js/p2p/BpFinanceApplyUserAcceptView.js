 ...
 * @createtime:
 */
BpFinanceApplyUserAcceptView = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				BpFinanceApplyUserAcceptView.superclass.constructor.call(this, {
							id : 'BpFinanceApplyUserAcceptView',
							title : '已受理',
							region : 'center',
							layout : 'border',
			...
			initUIComponents : function() {
				...
				this.gridPanel=new HT.GridPanel({
					...
					url : __ctxPath + "/p2p/personListBpFinanceApplyUser.do?state=2",
					fields : [{
									name : 'loanId',
									type : 'int'
								}
			...
			updateSelRs : function(obj,e) {
				...
				if (rows.length == 0) {
					...
					if(btest1=="受理"){
						...
						rowState='5';
					}else if(btest1=="不合格，结束项目"){
						rowState='3';
					}else if(btest1=="通过审核"){
						rowState='4'
					}
					Ext.Msg.confirm('信息确认', '是否'+(rowState=='3'?'结束项目':btest1), function(btn) {
       					 if (btn == 'yes') {
							Ext.Ajax.request({
