...
public class FinanceFundIntentListProtw1 {
	/**
	 * 贷款贷出
	 */
	 ...
	 * @return
	 */
	//到期日期减1(例如放款日期为2013-10-01 ,到期日期为2013-11-01) 算头又算尾
	public static List<SlFundIntent> getFundIntentDefaultList(String calcutype,
		...
		if (null ==isHaveLending ||isHaveLending.equals("yes")) { //像提前还款什么的都是不需要生成放款记录
			sf.setFundType(FinanceFundIntentListProtw1.Financingborrow);// 资金类型
			...
			if(!isStartDatePay.equals("1")){//按实际放款日对日还款
				if(isInterestByOneTime==0){//不一次性首付利息
		    	   for (int i = 1; i <=payintentPeriod; i++) {
		    		   ...
		    		   sf1 =calculslfundintent(FinanceFundIntentListProtw1.FinancingInterest,            // 资金类型
		    				   intentDate,  //计划日期
		    		   ...
		    		   sf =calculslfundintent(FinanceFundIntentListProtw1.FinancingRepay,            // 资金类型
					   ...
					   sf2 = calculslfundintent(FinanceFundIntentListProtw1.FinancingconsultationMoney,            // 资金类型
							   intentDate,  //计划日期
							   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,managementConsultingOfRate),BigDecimal.valueOf(0),  i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   sf3 = calculslfundintent(FinanceFundIntentListProtw1.FinancingserviceMoney,            // 资金类型
							   intentDate,  //计划日期
							   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,financeServiceOfRate),BigDecimal.valueOf(0),  i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
	    		   ...
	    		   if(isPreposePayAccrual == 1){//前置付息
	    			 ...
	    			 if(sf1.getPayMoney().compareTo(new BigDecimal(0))!=0){
					   ...
					   sf2 = calculslfundintent(FinanceFundIntentListProtw1.FinancingconsultationMoney,            // 资金类型
							   intentDate,  //计划日期
							   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(payintentPeriod)),BigDecimal.valueOf(0),  1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   sf3 = calculslfundintent(FinanceFundIntentListProtw1.FinancingserviceMoney,            // 资金类型
							   intentDate,  //计划日期
							   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(payintentPeriod)),BigDecimal.valueOf(0),  1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType         // 收入金额
								);
					   if(sf2.getPayMoney().compareTo(new BigDecimal(0))!=0){
					    list.add(sf2);
					   }
				   ...
				   for (int i = 1; i <=payintentPeriod; i++) {
		    		   if(isPreposePayAccrual ==0 && i==payintentPeriod){//前置付息
						   ...
						   sf2 = calculslfundintent(FinanceFundIntentListProtw1.FinancingconsultationMoney,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i),  //计划日期
								   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(payintentPeriod)),BigDecimal.valueOf(0),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						   sf3 = calculslfundintent(FinanceFundIntentListProtw1.FinancingserviceMoney,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i),  //计划日期
								   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(payintentPeriod)),BigDecimal.valueOf(0),  payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType         // 收入金额
									);
						   if(sf2.getPayMoney().compareTo(new BigDecimal(0))!=0){
						    list.add(sf2);
						   }
			...
			if(!isStartDatePay.equals("1")){//按实际放款日对日还款
				if(isInterestByOneTime==0){//不一次性首付利息
		    	   for (int i = 1; i <=payintentPeriod; i++) {
					   ...
					   sf2 = calculslfundintent(FinanceFundIntentListProtw1.FinancingconsultationMoney,            // 资金类型
							   intentDate,  //计划日期
							   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(30)),BigDecimal.valueOf(0),  i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   sf3 = calculslfundintent(FinanceFundIntentListProtw1.FinancingserviceMoney,            // 资金类型
							   intentDate,  //计划日期
							   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(30)),BigDecimal.valueOf(0),  i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   if(sf2.getPayMoney().compareTo(new BigDecimal(0))!=0){
					    list.add(sf2);
					   }
	    		   ...
	    		   if(isPreposePayAccrual == 1){//前置付息
	    			 ...
	    			 if(sf1.getPayMoney().compareTo(new BigDecimal(0))!=0){
					   ...
					   sf2 = calculslfundintent(FinanceFundIntentListProtw1.FinancingconsultationMoney,            // 资金类型
							   intentDate,  //计划日期
							   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)),BigDecimal.valueOf(0),  1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   sf3 = calculslfundintent(FinanceFundIntentListProtw1.FinancingserviceMoney,            // 资金类型
							   intentDate,  //计划日期
							   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)),BigDecimal.valueOf(0),  1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   if(sf2.getPayMoney().compareTo(new BigDecimal(0))!=0){
					    list.add(sf2);
					   }
				   ...
				   for (int i = 1; i <=payintentPeriod; i++) {
		    		   ...
		    		   if(isPreposePayAccrual == 0 && i==payintentPeriod){
						   ...
						   sf2 = calculslfundintent(FinanceFundIntentListProtw1.FinancingconsultationMoney,            // 资金类型
								   DateUtil.addMonthsToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), payintentPeriod),  //计划日期
								   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)),BigDecimal.valueOf(0),  payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						   sf3 = calculslfundintent(FinanceFundIntentListProtw1.FinancingserviceMoney,            // 资金类型
								   DateUtil.addMonthsToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), payintentPeriod),  //计划日期
								   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)),BigDecimal.valueOf(0),  payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						   if(sf2.getPayMoney().compareTo(new BigDecimal(0))!=0){
						    list.add(sf2);
						   }
			...
			if(!isStartDatePay.equals("1")){//按实际放款日对日还款
				if(isInterestByOneTime==0){//不一次性首付利息
		    	   for (int i = 1; i <=payintentPeriod; i++) {
					   ...
					   sf2 = calculslfundintent(FinanceFundIntentListProtw1.FinancingconsultationMoney,            // 资金类型
							   intentDate,  //计划日期
							   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(3)),BigDecimal.valueOf(0),  i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   sf3 = calculslfundintent(FinanceFundIntentListProtw1.FinancingserviceMoney,            // 资金类型
							   intentDate,  //计划日期
							   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(3)),BigDecimal.valueOf(0),  i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   if(sf2.getPayMoney().compareTo(new BigDecimal(0))!=0){
					    list.add(sf2);
					   }
	    		   ...
	    		   if(isPreposePayAccrual == 1){//前置付息
	    			 ...
	    			 list.add(sf1);
					   ...
					   sf2 = calculslfundintent(FinanceFundIntentListProtw1.FinancingconsultationMoney,            // 资金类型
							   intentDate,  //计划日期
							   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)).multiply(new BigDecimal(3)),BigDecimal.valueOf(0),  1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   sf3 = calculslfundintent(FinanceFundIntentListProtw1.FinancingserviceMoney,            // 资金类型
							   intentDate,  //计划日期
							   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)).multiply(new BigDecimal(3)),BigDecimal.valueOf(0),  1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   if(sf2.getPayMoney().compareTo(new BigDecimal(0))!=0){
					    list.add(sf2);
					   }
			...
			if(!isStartDatePay.equals("1")){//按实际放款日对日还款
				if(isInterestByOneTime==0){//不一次性首付利息
		    	   for (int i = 1; i <=payintentPeriod; i++) {
					   ...
					   sf2 = calculslfundintent(FinanceFundIntentListProtw1.FinancingconsultationMoney,            // 资金类型
							   intentDate,  //计划日期
							   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(dayOfEveryPeriod)),BigDecimal.valueOf(0),  i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   sf3 = calculslfundintent(FinanceFundIntentListProtw1.FinancingserviceMoney,            // 资金类型
							   intentDate,  //计划日期
							   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(dayOfEveryPeriod)),BigDecimal.valueOf(0),  i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   if(sf2.getPayMoney().compareTo(new BigDecimal(0))!=0){
					    list.add(sf2);
					   }
	    		   ...
	    		   if(isPreposePayAccrual == 1){//前置付息
	    			 ...
	    			 list.add(sf1);
					   ...
					   sf2 = calculslfundintent(FinanceFundIntentListProtw1.FinancingconsultationMoney,            // 资金类型
							   intentDate,  //计划日期
							   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(dayOfEveryPeriod)).multiply(new BigDecimal(payintentPeriod)),BigDecimal.valueOf(0),  1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   sf3 = calculslfundintent(FinanceFundIntentListProtw1.FinancingserviceMoney,            // 资金类型
							   intentDate,  //计划日期
							   FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(dayOfEveryPeriod)).multiply(new BigDecimal(payintentPeriod)),BigDecimal.valueOf(0),  1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   if(sf2.getPayMoney().compareTo(new BigDecimal(0))!=0){
					    list.add(sf2);
					   }
	           ...
	           for (int i = 0; i < payintentPeriod; i++) {
	        	   Date intentDate=null;
					...
					/* }else{
					 ...
					 sf1 = calculslfundintent(FinanceFundIntentListProtw1.FinancingInterest,            // 资金类型
							 intentDate,  //计划日期
							 FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,accrual),BigDecimal.valueOf(0),i+1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType 	);        // 收入金额
					 if(sf1.getPayMoney().compareTo(new BigDecimal(0))!=0){
					 list.add(sf1);
					 }
					 sf2 = calculslfundintent(FinanceFundIntentListProtw1.FinancingconsultationMoney,            // 资金类型
					    		intentDate,  //计划日期
					    		FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,managementConsultingOfRate),BigDecimal.valueOf(0),  i+1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					 sf3 = calculslfundintent(FinanceFundIntentListProtw1.FinancingserviceMoney,            // 资金类型
								intentDate,  //计划日期
								FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,financeServiceOfRate),BigDecimal.valueOf(0),  i+1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
				...
				if(isPreposePayAccrual==1){
				 ...
				 sf1 = calculslfundintent(FinanceFundIntentListProtw1.FinancingInterest,            // 资金类型
						 intentDate,  //计划日期
						 FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,accrual).multiply(new BigDecimal(payintentPeriod)),BigDecimal.valueOf(0),qs,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType 	);        // 收入金额
				 if(sf1.getPayMoney().compareTo(new BigDecimal(0))!=0){
				 list.add(sf1);
				 }
				 sf2 = calculslfundintent(FinanceFundIntentListProtw1.FinancingconsultationMoney,            // 资金类型
				    		intentDate,  //计划日期
				    		FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(payintentPeriod)),BigDecimal.valueOf(0),  qs,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
							);
				 sf3 = calculslfundintent(FinanceFundIntentListProtw1.FinancingserviceMoney,            // 资金类型
							intentDate,  //计划日期
							FinanceFundIntentListProtw1.calculAccrualnew(projectMoney,financeServiceOfRate).multiply(new BigDecimal(payintentPeriod)),BigDecimal.valueOf(0),  qs,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
							);
			...
			if(payintentPeriod>0){
			 ...
			 if(isStartDatePay.equals("1")){ //固定在某日
				...
				// if(payintentPeriod>1){
				 String firstdays=Integer.valueOf(DateUtil.getDaysBetweenDate( date1,sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1)))+1).toString();
				 	sf1 = calculslfundintent(FinanceFundIntentListProtw1.FinancingInterest,            // 资金类型
				 			isPreposePayAccrual==0?DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1):DateUtil.parseDate(date1,"yyyy-MM-dd"),  //计划日期
				 					FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(firstdays)),      // 收入金额
				 ...
				 sf1 = calculslfundintent(FinanceFundIntentListProtw1.FinancingInterest,            // 资金类型
						 intentDate,  //计划日期
						 FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30)),
				 ...
				 sf1 = calculslfundintent(FinanceFundIntentListProtw1.FinancingInterest,            // 资金类型
						 intDate,  //计划日期
						 FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(enddays)),      // 收入金额
					...
					preperiodsurplus=preperiodsurplus.subtract(sf.getPayMoney());
					 ...
					 BigDecimal financeServiceOfRateMoney=new BigDecimal(0);
						 ...
						 accrualMoney=accrualMoney.add(FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(firstdays)));
						 BigDecimal  managementConsultingOfRate1=managementConsultingOfRate.multiply(new BigDecimal(firstdays));
						 ...
						 for (int i = 2; i <payintentPeriod; i++) {
							 accrualMoney=accrualMoney.add(FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30)));
						 ...
						 BigDecimal managementConsultingOfRate2=managementConsultingOfRate.multiply(new BigDecimal(enddays));
						 BigDecimal financeServiceOfRate2=financeServiceOfRate.multiply(new BigDecimal(enddays));
						 accrualMoney=accrualMoney.add(FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(enddays)));
				 ...
				 if(isInterestByOneTime==0){
					 for (int i = 1; i < payintentPeriod+1; i++) {
						 ...
						 sf1 = calculslfundintent(FinanceFundIntentListProtw1.FinancingInterest,            // 资金类型
								intentDate,  //计划日期
								FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30)),BigDecimal.valueOf(0),
		...
		if (accrualType.equals("sameprincipalandInterest") && payaccrualType.equals("monthPay")) {// 月尾
		     BigDecimal periodtimemoney=FinanceFundIntentListProtw1.periodtimemoney(accrual.multiply(new BigDecimal(30)),projectMoney,payintentPeriod).divide(new BigDecimal(1),2,BigDecimal.ROUND_HALF_UP);
		     BigDecimal preperiodsurplus=projectMoney;
		     if(isStartDatePay.equals("1")){ //固定在某日
		    	 if(isInterestByOneTime==0){
			    	 String firstdays=Integer.valueOf(DateUtil.getDaysBetweenDate( date1,sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1)))+1).toString();
			    	 ...
			    	 sf1 = calculslfundintent(FinanceFundIntentListProtw1.FinancingInterest,            // 资金类型
			    			 isPreposePayAccrual==0?DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1):DateUtil.parseDate(date1,"yyyy-MM-dd"),  //计划日期
			    					 FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(firstdays)), BigDecimal.valueOf(0),     // 收入金额
			    	  ...
			    	  BigDecimal firstperiodtimemoney=periodtimemoney.add(sf1.getIncomeMoney().subtract(FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30))));
					    sf =calculslfundintent(FinanceFundIntentListProtw1.FinancingRepay,            // 资金类型
								DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1),  //计划日期
					...
					sf1 = calculslfundintent(FinanceFundIntentListProtw1.FinancingInterest,            // 资金类型
							isPreposePayAccrual==0?DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), i):DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), i-1),  //计划日期
									FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30)),BigDecimal.valueOf(0),      // 收入金额
				    ...
				    sf =calculslfundintent(FinanceFundIntentListProtw1.FinancingRepay,            // 资金类型
							DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), i),  //计划日期
							periodtimemoney.subtract(FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30))),BigDecimal.valueOf(0),        // 收入金额
							i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,String.valueOf(payintentPerioDate),accrualType
				    );
				    if(sf1.getPayMoney().compareTo(new BigDecimal(0))!=0){
						list.add(sf1);
					...
					sf1 = calculslfundintent(FinanceFundIntentListProtw1.FinancingInterest,            // 资金类型
							isPreposePayAccrual==0?DateUtil.addMonthsToDate(DateUtil.parseDate(date1), payintentPeriod):DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), payintentPeriod-1),  //计划日期
									FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(enddays)),BigDecimal.valueOf(0),      // 收入金额
								payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,String.valueOf(payintentPerioDate),accrualType
								);
			    	  BigDecimal endperiodtimemoney=periodtimemoney.subtract((FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30))).subtract(sf1.getIncomeMoney()));
					    sf =calculslfundintent(FinanceFundIntentListProtw1.FinancingRepay,            // 资金类型
					    		DateUtil.addMonthsToDate(DateUtil.parseDate(date1), payintentPeriod),  //计划日期
					...
					//}
			    	 ...
			    	 String firstdays=Integer.valueOf(DateUtil.getDaysBetweenDate( date1,sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1)))).toString();
			    	 accrualMoney=accrualMoney.add(FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus1,accrual).multiply(new BigDecimal(firstdays)));
			    	 BigDecimal managementConsultingOfRate1=managementConsultingOfRate.multiply(new BigDecimal(firstdays));
			    	 managementConsultingOfRateMoney=managementConsultingOfRateMoney.add(projectMoney.multiply(managementConsultingOfRate1));
			    	 BigDecimal financeServiceOfRate1=financeServiceOfRate.multiply(new BigDecimal(firstdays));
			    	 financeServiceOfRateMoney=financeServiceOfRateMoney.add(projectMoney.multiply(financeServiceOfRate1));
						...
						preperiodsurplus1=preperiodsurplus1.subtract(periodtimemoney.subtract(FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus1,accrual).multiply(new BigDecimal(30))));
						
						
						
					for (int i = 2; i < payintentPeriod; i++) {
						accrualMoney=accrualMoney.add(FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus1,accrual).multiply(new BigDecimal(30)));
						managementConsultingOfRateMoney=managementConsultingOfRateMoney.add(projectMoney.multiply(managementConsultingOfRate).multiply(new BigDecimal(30)));
						financeServiceOfRateMoney=financeServiceOfRateMoney.add(projectMoney.multiply(financeServiceOfRate).multiply(new BigDecimal(30)));
					...
					String enddays=Integer.valueOf(DateUtil.getDaysBetweenDate( sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), payintentPeriod-1)),sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(date1), payintentPeriod)))).toString();
					accrualMoney=accrualMoney.add(FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus1,accrual).multiply(new BigDecimal(enddays)));
					BigDecimal managementConsultingOfRate2=managementConsultingOfRate.multiply(new BigDecimal(enddays));
					managementConsultingOfRateMoney=managementConsultingOfRateMoney.add(projectMoney.multiply(managementConsultingOfRate2));
					BigDecimal financeServiceOfRate2=financeServiceOfRate.multiply(new BigDecimal(enddays));
					financeServiceOfRateMoney=financeServiceOfRateMoney.add(projectMoney.multiply(financeServiceOfRate2));
					...
					if(isPreposePayAccrual==1){
					  ...
					  sf =calculslfundintent(FinanceFundIntentListProtw1.FinancingRepay,            // 资金类型
						...
						preperiodsurplus=preperiodsurplus.subtract(periodtimemoney.subtract(FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30))));
						for (int i = 2; i < payintentPeriod; i++) {
					    ...
					    sf =calculslfundintent(FinanceFundIntentListProtw1.FinancingRepay,            // 资金类型
					    		DateUtil.addMonthsToDate(DateUtil.parseDate(date1), payintentPeriod),  //计划日期
								periodtimemoney.subtract(FinanceFundIntentListProtw1.calculAccrualnew(preperiodsurplus1,accrual).multiply(new BigDecimal(enddays))),BigDecimal.valueOf(0),        // 收入金额
								payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,String.valueOf(payintentPerioDate),accrualType
					    );
						list.add(sf);
