var ReportPreviewLoanLimitInterval = function(reportId, title,reportKey) {
			    ...
			    items : [
			    {
			      ...
			      listeners:{
			      	...
			      	'afterrender':function(combox){
			      		...
			      		st.on('load',function(){
			      			combox.setValue(st.getAt(0).data.companyId)
			      		})
			...
			pdfButton.on('click', function(xlsButton) {
				...
				if (formPanel.getForm().isValid()) {
					...
					formPanel.getForm().submit({
						waitMsg : '正在提交查询',
						url : __ctxPath + '/system/submitLoanLimitIntervalReportTemplate.do?reportKey='+reportKey,
						method : 'post',
