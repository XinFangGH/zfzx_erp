var addMortgageIndustry = function(piKey,personTypeParams,assuretypeidParams,refreshMortgageGridStore,businessType){
	var anchor = '100%';
	
	var showIndustryWin = function(){
		...
		items : [{
			...
			handler : function() {
				...
				if(mortgageName == ""){
					...
					panel_addIndustry.getForm().submit({
						method : 'POST',
						waitTitle : '连接',
						waitMsg : '消息发送中...',
						success : function(form, action) {
							Ext.ux.Toast.msg('操作信息', '保存成功!');
							Ext.getCmp('win_addIndustry').destroy();
		...
		var panel_addIndustry = new Ext.form.FormPanel({
		id :'addIndustry',
		url : __ctxPath +'/credit/mortgage/addIndustry.do',
		monitorValid : true,
					...
					items : [{
						...
						onTriggerClick : function(){
							if(businessType=='Financing'){
								selectSlCompanyMain(selectSlCompanyInd);
							}else{
						...
						onTriggerClick : function(){
							if(businessType=='Financing'){
								selectSlPersonMain(selectSlPersonInd);
							}else{
	...
	var window_addIndustry = new Ext.Window({
		id : 'win_addIndustry',
		title :'新增工业用地信息',
		iconCls : 'btn-add',
		...
		modal : true,
		buttonAlign: 'right',
		items : [panel_addIndustry],
		listeners : {
			'beforeclose' : function(){
				if(panel_addIndustry.getForm().isDirty()){
					Ext.Msg.confirm('操作提示','是否保存当前新添加的数据?',function(btn){
						if(btn=='yes'){
							panel_addIndustry.getTopToolbar().items.itemAt(0).handler.call() ;
						}else{
							panel_addIndustry.getForm().reset() ;
							window_addIndustry.destroy() ;
						}
					}) ;
					return false ;
				}
