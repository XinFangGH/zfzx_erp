 ...
 * @createtime:
 */
//SlApprovalFormView.jStore_RiskReport;
SlApprovalFormView = Ext.extend(Ext.Panel, {
	...
	constructor : function(_cfg) {
	  ...
	  if(typeof(_cfg.uploadHidden)!="undefined")
      {
            this.uploadHidden=_cfg.uploadHidden;
      }else{
      	 this.uploadHidden=false
      }
	  

	  if(typeof(_cfg.downloadHidden)!="undefined")
      {
            this.downloadHidden=_cfg.downloadHidden;
      }else{
		...
		// 调用父类构造
		SlApprovalFormView.superclass.constructor.call(this, {
	...
	initUIComponents : function() {
		...
		RunNtkOfficePanelForRiskReport = function(projId,businessType,bln,ApprovalForm,templettype){
			Ext.Ajax.request({
						...
						Ext.Ajax.request({
							...
							success : function(response, request) {
								...
								if(objfile.success==true){
									...
									Ext.ux.Toast.msg('提示', '可能还未上传审批表模版！');
								}
			...
			items : [
				{
					iconCls : 'sldownIcon',
					text:'下载模板',
					hidden : this.downloadHidden,
					scope:this,
					handler : function(){
						this.downloadApprovalForm(this.ApprovalForm,this.businessType)
					}
				},'-',{
					text:'上传',
					iconCls : 'slupIcon',
					hidden : this.uploadHidden,
					scope:this,
					handler : function() {
						this.uploadRiskReportForThisPanel(this.projId,this.businessType,true,this.ApprovalForm,this.templettype,this.grid_RiskReportPanel,this.setname)
					}
				}
			]
		});
...
//					return '<a title="下载审批表" style ="TEXT-DECORATION:underline;cursor:pointer" onclick="SlApprovalFormView.uploadRiskReportForThisPanel(\''+this.projId+'\',\''+this.businessType+'\','+false+',\''+this.ApprovalForm+'\','+this.templettype+')" >下载</a>';
					return '<a title="下载审批表" style ="TEXT-DECORATION:underline;cursor:pointer" onclick="downReportFileJS('+r.get('fileid')+')" >下载</a>';
				}
			}/*, {
				header : '在线编辑',
				dataIndex : 'fileid',
				...
				scope :this,
				renderer : function(val, m, r) {
					return '<a title="在线编辑审批表" style ="TEXT-DECORATION:underline;cursor:pointer" onclick="RunNtkOfficePanelForRiskReport(\''+this.projId+'\',\''+this.businessType+'\','+false+',\''+this.ApprovalForm+'\','+this.templettype+')" >编辑</a>';
				}
			}, {
				header : '在线查看',
				dataIndex : 'fileid',
				...
				renderer : function(val, m, r) {
					if(this.isHidden_riskReport==true){
						return '<a title="在线编辑审批表" style ="TEXT-DECORATION:underline;cursor:pointer" onclick="RunNtkOfficePanelForRiskReport(\''+this.projId+'\',\''+this.businessType+'\','+true+',\''+this.ApprovalForm+'\','+this.templettype+')" >查看</a>';
					}else if(!uploadHidden){
						return '<a title="在线编辑审批表" style ="TEXT-DECORATION:underline;cursor:pointer" onclick="RunNtkOfficePanelForRiskReport(\''+this.projId+'\',\''+this.businessType+'\','+true+',\''+this.ApprovalForm+'\','+this.templettype+')" >查看</a>';
					}else{
						return '';
					}
			...
			listeners : {
				...
				'cellclick' : function(grid,row,col,e){
					...
					if(col == 13){
						...
						this.reUploadRiskReportForThisPanel(this.projId,this.businessType,true,fileid,this.ApprovalForm,this.templettype,grid,this.setname)
					}
				}
			}
		});
	...
	uploadRiskReportForThisPanel : function(projId,businessType,displayDel,ApprovalForm,templettype,panel,setname){
		...
		Ext.Ajax.request({
			...
			success : function(response, request) {
				...
				if(obj.success==true){
					...
					if(displayDel == true){
						uploadReportJS('上传/下载审批表',"typeisfile",olymark,1,null,null,reloadjStore_RiskReport,projId,businessType,setname);
					}else{
						downIsFileJS("typeisfile",olymark,projId,businessType);
					}
					
				}else{
					Ext.ux.Toast.msg('状态', '可能未上传审批表模板！');
				}
	...
	downloadApprovalForm : function(ApprovalForm,businessType){
		Ext.Ajax.request({
			...
			success : function(response, request) {
				var obj = Ext.util.JSON.decode(response.responseText);
				if(obj.success==true){
					window.open(__ctxPath+'/contract/ajaxGetReportTemplateDocumentTemplet.do?mark='+ApprovalForm+'&businessType='+businessType,'_blank');
	...
	//		window.location.href =__ctxPath+'/credit/document/ajaxGetReportTemplate.do?mark='+ApprovalForm+'&templettype='+templettype;
			//window.open(__ctxPath+'/credit/document/ajaxGetReportTemplate.do?mark='+ApprovalForm+'&businessType='+businessType,'_blank');
		},
	reUploadRiskReportForThisPanel : function(projId,businessType,displayDel,fileid,ApprovalForm,templettype,panel,setname){
		var reloadjStore_RiskReport = function(){panel.getStore().reload()};
		//var mark = "cs_document_templet";
		Ext.Ajax.request({
			url : __ctxPath+'/contract/getByOnlymarkForUploadDocumentTemplet.do',
			...
			success : function(response, request) {
				...
				if(obj.success==true){
					var olymark = "cs_document_templet."+obj.data.id;
					reUploadReportJS('上传/下载调查报告',"typeisfile",olymark,2,null,null,reloadjStore_RiskReport,projId,businessType,fileid,setname);
