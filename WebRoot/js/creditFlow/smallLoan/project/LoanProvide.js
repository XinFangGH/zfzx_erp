 ...
 * @createtime:
 */
LoanProvide = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		LoanProvide.superclass.constructor.call(this, {
			id : 'LoanProvide',
			title : '贷款发放',
			region : 'center',
			layout : 'border',
			iconCls :'btn-tree-team10',
	...
	initUIComponents : function() {
		...
		this.topbar = new Ext.Toolbar({
			items : [{
						...
						xtype : 'button',
						scope : this,
						//hidden : isGranted('_loanProvide_flow') ? false : true,
		...
		this.gridPanel = new HT.GridPanel( {
			...
			notmask :true,
			tbar:this.topbar,
			url : __ctxPath + "/flow/getByProcessNameNodeKeyOfCommonTask.do?processName=smallLoanCommonFlow&flowNodeKey=slcfLoanProvide&searchType=byUserId",
			fields : [ {
				name : 'task.taskId',
				type : 'int'
			}, 'task.dueDate',
