var add_button_o = new Ext.Button({
		    text : '添加租赁材料项',
		    tooltip : '添加选中租赁材料的租赁材料项',
		    hidden:isGranted('_create_leaseFinance_materials')?false:true,
		    iconCls : 'addIcon'
}); 
var update_button_o = new Ext.Button({
		    text : '修改租赁材料项',
		    tooltip : '修改选中的租赁材料项',
		    hidden:isGranted('_edit_leaseFinance_materials')?false:true,
		    iconCls : 'updateIcon'
});
var delete_button_o = new Ext.Button({
			text : '删除租赁材料项',
			tooltip : '从数据库中完全删除选中租赁材料项',
			hidden:isGranted('_remove_leaseFinance_materials')?false:true,
			iconCls : 'deleteIcon'
});
...
var update_button_r = new Ext.Button({
						...
						handler : function() {
							...
							if (gselectid == '0') {
								Ext.ux.Toast.msg('操作提示', '请选择您要修改的租赁材料项！');
								return;
							}
							Ext.Ajax.request({
								url : __ctxPath+ '/materials/getOurProcreditMaterialsEnterprise.do',
