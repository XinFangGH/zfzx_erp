/**
 * @author lisl
 * @class flDesignSupervisionManagePlan
 * @extends Ext.Window
 * @description 监管计划
 * @company 智维软件
 * @createtime:
 */
flDesignSupervisionManagePlan = Ext.extend(Ext.Window, {
	isHidden : false,
	isHeightNull:false,
	titile:"贷后监管",
	constructor : function(_cfg) {
		...
		if(typeof(_cfg.heightValue)!="undefined"){
			this.heightValue = _cfg.heightValue;
		}
		if(typeof(_cfg.title)!="undefined"){
			this.title = _cfg.title;
		}
		this.initUIComponents();
		flDesignSupervisionManagePlan.superclass.constructor.call(this, {
			title : this.title,
			layout : 'anchor',
			width : 590,
			height : 395,
	...
	initUIComponents : function() {
		...
		this.gridPanel = new HT.EditorGridPanel({
			...
			region : 'center',
			tbar : this.topbar,
			height :!this.isHeightNull?(this.isHidden?362:331):null,
			autoHeight:true,
			autoScroll : true,
			showPaging : false,
			clicksToEdit : 1,
	...
	detailGlobalSupervison : function() {
		...
		if(superviseManageId != null && superviseManageStatus == 1) {
			new GlobalSupervisonRecordView({projectId : this.projectId,superviseManageId : superviseManageId,businessType:this.businessType}).show();
		}else {
