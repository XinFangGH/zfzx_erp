//定量的tree
function setCatalogMenuFun2(node,e,centerPanel,ptype){
		var setCatalogMenu; 
		...
		if((nodeId == null)){ //当前节点为根节点
			setCatalogMenu = new Ext.menu.Menu({
				items : [{
					...
					handler : function (){
						addCatalogType2(false,node,ptype);
					}
				}]
			})
			...
			setCatalogMenu = new Ext.menu.Menu({
				items :[{
					...
					handler : function (){
						updateCatalogType2(node,ptype);
					}
				},"-",{
					text : '删除评估要素',
					hidden : isGranted('_yaosu_delete')?false:true,
					handler : function (){
						deleteCatalogType2(node,centerPanel,ptype);
					}
				}]
			})
			...
			setCatalogMenu = new Ext.menu.Menu({
				items :[/*{
					...
					handler : function (){
						addCatalogType2(true,node,ptype);
					}
				},"-",{
					text : '修改目录类型',
	...
	//添加目录，添加指标
	function addCatalogType2(isLaef,node,ptype){
	...
	function updateCatalogType2(node,ptype){
		var id = node.id;
		var text = node.text;
	...
	function deleteCatalogType2(node,centerPanel){
		var id = node.id;
