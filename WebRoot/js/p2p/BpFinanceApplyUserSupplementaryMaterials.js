 ...
 * @createtime:
 */
BpFinanceApplyUserSupplementaryMaterials = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				BpFinanceApplyUserSupplementaryMaterials.superclass.constructor.call(this, {
							id : 'BpFinanceApplyUserSupplementaryMaterials',
							title : '借款项目审批通过，请补充材料',
							region : 'center',
							layout : 'border',
			...
			initUIComponents : function() {
				...
				this.topbar = new Ext.Toolbar({
						items : [{
									...
									if (rows.length == 0) {
										...
										Ext.Msg.confirm('信息确认', '资料是否已上传', function(btn) {
					       					 if (btn == 'yes') {
												Ext.Ajax.request({
													url:__ctxPath + '/p2p/saveBpFinanceApplyUser.do',
													params:{'loanId':rows[0].data.loanId,'state':8},
				...
				this.gridPanel=new HT.GridPanel({
					...
					url : __ctxPath + "/p2p/personListBpFinanceApplyUser.do?state=7",
					fields : [{
									name : 'loanId',
									type : 'int'
								}
