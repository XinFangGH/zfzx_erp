package com.zhiwei.credit.service.p2p.loan.impl;
/*
 *  北京互融时代软件有限公司   -- http://www.zhiweitime.com
 *	Copyright @ 2004 - 2010 Yuseen.com all rights reserved.京ICP备 05007290 号
*/
import com.zhiwei.core.service.impl.BaseServiceImpl;
import com.zhiwei.credit.dao.p2p.loan.P2pLoanProductDao;
import com.zhiwei.credit.model.p2p.loan.P2pLoanProduct;
import com.zhiwei.credit.service.p2p.loan.P2pLoanProductService;

/**
 * 
 * @author 
 *
 */
public class P2pLoanProductServiceImpl extends BaseServiceImpl<P2pLoanProduct> implements P2pLoanProductService{
	@SuppressWarnings("unused")
	private P2pLoanProductDao dao;
	
	public P2pLoanProductServiceImpl(P2pLoanProductDao dao) {
		super(dao);
		this.dao=dao;
	}

}