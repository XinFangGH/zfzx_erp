...
Ext.extend(Ext.ux.grid.ProgressColumn, Ext.util.Observable, {
  ...
  renderer : function(v, p, record) {
    ...
    var text='</div><div class="x-progress-text {0}" style="width:100%;" id="{1}">{2}</div></div>';
    if(v>0 && v<0.01){
	    text = String.format(text,textClass, Ext.id(), "<0.01" + this.textPst);
    }else{
    	text = String.format(text,textClass, Ext.id(), v + this.textPst);
    }
