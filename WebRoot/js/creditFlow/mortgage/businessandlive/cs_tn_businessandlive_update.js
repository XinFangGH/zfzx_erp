var updateBusinessAndLive = function(mortgageid,refreshMortgageGridStore,businessType){
	var anchor = '100%';
	
	var showBusinessAndLiveInformation = function(MortgageUpdateData){
		var panel_updateBusinessAndLive = new Ext.form.FormPanel({
			id :'updateBusinessAndLive',
			url : __ctxPath +'/credit/mortgage/updateBusinessAndLive.do',
			monitorValid : true,
			...
			items : [{
				...
				items : [{
					...
					onTriggerClick : function(){
						if(businessType=='Financing'){
							selectSlCompanyMain(selectSlCompanyBusLiveUpdate);
						}else{
					...
					onTriggerClick : function(){
						if(businessType=='Financing'){
							selectSlPersonMain(selectSlPersonBusLiveUpdate);
						}else{
	            ...
	            items : [{
					...
					items : [{
						...
						maxLength : 50,
						maxLengthText : '最大输入长度50',
						value : MortgageUpdateData.vProjMortBusAndLive.address,
					...
					items : [{
						...
						maxLength : 50,
						maxLengthText : '最大输入长度50',
						value : MortgageUpdateData.vProjMortBusAndLive.certificatenumber,
						name : 'procreditMortgageBusinessandlive.certificatenumber'
					},{
						fieldLabel : '产权人',
						maxLength : 50,
						maxLengthText : '最大输入长度50',
						value : MortgageUpdateData.vProjMortBusAndLive.propertyperson,
					...
					items : [{
						...
						value : MortgageUpdateData.vProjMortBusAndLive.descriptionId,
						fieldLabel : "地段描述",
						editable :false,
						...
						value : MortgageUpdateData.vProjMortBusAndLive.registerInfoId,
						fieldLabel : "登记情况",
						editable :false,
					...
					items : [{
						...
						maxLength : 23,
						maxLengthText : '最大输入长度23',
						value : MortgageUpdateData.vProjMortBusAndLive.anticipateacreage==0?'':MortgageUpdateData.vProjMortBusAndLive.anticipateacreage,
						name : 'procreditMortgageBusinessandlive.anticipateacreage'
					},{
						fieldLabel : '土地抵质押贷款余额.万元',
						maxLength : 23,
						maxLengthText : '最大输入长度23',
						value : MortgageUpdateData.vProjMortBusAndLive.mortgagesbalance==0?'':MortgageUpdateData.vProjMortBusAndLive.mortgagesbalance,
						name : 'procreditMortgageBusinessandlive.mortgagesbalance'
					},{
						fieldLabel : '同等土地成交单价.元/㎡',
						maxLength : 23,
						maxLengthText : '最大输入长度23',
						value : MortgageUpdateData.vProjMortBusAndLive.exchangepriceground==0?'':MortgageUpdateData.vProjMortBusAndLive.exchangepriceground,
						name : 'procreditMortgageBusinessandlive.exchangepriceground'
					},{
						fieldLabel : '同等住宅成交单价.元/㎡',
						maxLength : 23,
						maxLengthText : '最大输入长度23',
						value : MortgageUpdateData.vProjMortBusAndLive.exchangepricehouse==0?'':MortgageUpdateData.vProjMortBusAndLive.exchangepricehouse,
					...
					items : [{
						...
						maxLength : 23,
						maxLengthText : '最大输入长度23',
						value : MortgageUpdateData.vProjMortBusAndLive.acreage==0?'':MortgageUpdateData.vProjMortBusAndLive.acreage,
						name : 'procreditMortgageBusinessandlive.acreage'
					},{
						fieldLabel : '租赁模型估值.万元',
						maxLength : 23,
						maxLengthText : '最大输入长度23',
						value : MortgageUpdateData.vProjMortBusAndLive.tenancyrangeprice==0?'':MortgageUpdateData.vProjMortBusAndLive.tenancyrangeprice,
						name : 'procreditMortgageBusinessandlive.tenancyrangeprice'
					},{
						fieldLabel : '销售模型估值.万元',
						maxLength : 23,
						maxLengthText : '最大输入长度23',
						value : MortgageUpdateData.vProjMortBusAndLive.venditionrangeprice==0?'':MortgageUpdateData.vProjMortBusAndLive.venditionrangeprice,
						name : 'procreditMortgageBusinessandlive.venditionrangeprice'
					},{
						fieldLabel : '模型估价.万元',
						maxLength : 23,
						maxLengthText : '最大输入长度23',
						value : MortgageUpdateData.vProjMortBusAndLive.modelrangeprice==0?'':MortgageUpdateData.vProjMortBusAndLive.modelrangeprice,
					...
					items : [{
						...
						maxLength : 23,
						maxLengthText : '最大输入长度23',
						value : MortgageUpdateData.vProjMortBusAndLive.rentpriceformonth==0?'':MortgageUpdateData.vProjMortBusAndLive.rentpriceformonth,
					...
					items : [{
						...
						maxLength : 23,
						maxLengthText : '最大输入长度23',
						value : MortgageUpdateData.vProjMortBusAndLive.exchangepricebusiness==0?'':MortgageUpdateData.vProjMortBusAndLive.exchangepricebusiness,
						name : 'procreditMortgageBusinessandlive.exchangepricebusiness'
					},{
		            	name : 'procreditMortgageBusinessandlive.id',
		            	value : MortgageUpdateData.vProjMortBusAndLive.id,
		            	xtype : 'hidden'
		            },{
				...
				items : [{
					...
					handler : function() {
						...
						if(mortgageName == ""){
							...
							panel_updateBusinessAndLive.getForm().submit({
								method : 'POST',
								waitTitle : '连接',
								waitMsg : '消息发送中...',
								success : function(form, action) {
									Ext.ux.Toast.msg('操作信息', '保存成功!');
									Ext.getCmp('win_updateBusinessAndLive').destroy();
		...
		var window_updateBusinessAndLive = new Ext.Window({
			id : 'win_updateBusinessAndLive',
			title :'修改商住用地信息',
			iconCls : 'btn-update',
			...
			modal : true,
			buttonAlign: 'right',
			items : [panel_updateBusinessAndLive],
			listeners : {
				'beforeclose' : function(){
					if(panel_updateBusinessAndLive.getForm().isDirty()){
						Ext.Msg.confirm('操作提示','数据被修改过,是否保存?',function(btn){
							if(btn=='yes'){
								panel_updateBusinessAndLive.getTopToolbar().items.itemAt(0).handler.call() ;
							}else{
								panel_updateBusinessAndLive.getForm().reset() ;
								window_updateBusinessAndLive.destroy() ;
							}
						}) ;
						return false ;
					}
		...
		window_updateBusinessAndLive.show();
	...
	Ext.Ajax.request({
		...
		success : function(response, request) {
			...
			if(obj.success==true){
				showBusinessAndLiveInformation(obj.data) ;
			}else{
