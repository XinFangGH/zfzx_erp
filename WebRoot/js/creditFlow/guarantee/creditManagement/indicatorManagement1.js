...
IndicatorManagement1 =Ext.extend(Ext.Panel,{
	...
	initUIComponents:function(){
			...
			var defaultTextfieldWidth = 200 ;//默认文本输入域宽度
			var fieldWidth = 150;
			var root = 'topics';
				...
				handler : function() {
					...
					var fPanel_add = new Ext.FormPanel({
						...
						buttons : [ {
							...
							formBind : true,
							handler : function() {
								var vDatas=getOptionGridDate1(fPanel_add.getCmpByName("gPanel_option"));
								if(vDatas && vDatas!=null){
									fPanel_add.getForm().submit({
										url: __ctxPath+'/creditFlow/creditmanagement/addIndicator.do',
										method : 'POST',
										waitTitle : '连接',
										waitMsg : '消息发送中...',
										params:{optionStr:vDatas},
										success : function() {
		...
		/**修改企业信息弹出开始*/
								var fPanel_update = new Ext.FormPanel({
									...
									buttons : [ {
										...
										formBind : true,
										handler : function() {
											var vDatas=getOptionGridDate1(fPanel_update.getCmpByName("gPanel_option"));
											if(vDatas && vDatas!=null){
			...
			this.centerPanel = new HT.GridPanel( {
				...
				columns:[{
						...
						renderer : function(value, metaData, record, rowIndex,colIndex, store) {
							if(record.data.optionStart.indexOf('null')==-1){
								return record.data.optionStart+record.data.optionEnd;
							}else{
								return value;
							}
						}
