...
public class PawnFundIntentListProtw {
	/**
	 * 贷款贷出
	 */
	public static final String ProjectLoadOut = "pawnPrincipalLending"; // 本金放贷

	/**
	 * 贷款收息
	 */
	public static final String ProjectLoadAccrual = "pawnLoanInterest"; // 利息收取
	/**
	 * 贷款收本
	 */
	public static final String ProjectLoadRoot = "pawnPrincipalRepayment"; // 贷款本金偿还
	
	/**
	 * 财务服务费用收取
	 */
	public static final String ProjectFinanceService = "pawnServiceMoney"; // 月费率
	/**
	 * @param calcutype
	 *            贷款方式 : 0为贷款;1为融资
	 * 
	...
	public static List<SlFundIntent> getFundIntentDefaultList(String calcutype,
		...
		if (null ==isHaveLending ||isHaveLending.equals("yes")) { //像提前还款什么的都是不需要生成放款记录
			sf.setFundType(PawnFundIntentListProtw.ProjectLoadOut);// 资金类型
		...
		if (accrualType.equals("singleInterest") && payaccrualType.equals("dayPay")) {// 日尾
	           ...
	           for (int i = 0; i < payintentPeriod; i++) {
	        	   Date intentDate=null;
					...
					/* }else{
					 ...
					 sf1 = calculslfundintent(PawnFundIntentListProtw.ProjectLoadAccrual,            // 资金类型
							 intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								PawnFundIntentListProtw.calculAccrualnew(projectMoney,accrual),i+1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType 	);        // 收入金额
					 if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
					 list.add(sf1);
					 }
					 sf2 = calculslfundintent(PawnFundIntentListProtw.ProjectFinanceService,            // 资金类型
					    		intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								PawnFundIntentListProtw.calculAccrualnew(projectMoney,managementConsultingOfRate),i+1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					/* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
								intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
				...
				if(isPreposePayAccrual==1){
				 ...
				 sf1 = calculslfundintent(PawnFundIntentListProtw.ProjectLoadAccrual,            // 资金类型
						 intentDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							PawnFundIntentListProtw.calculAccrualnew(projectMoney,accrual).multiply(new BigDecimal(payintentPeriod)),qs,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType 	);        // 收入金额
				 if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
				 list.add(sf1);
				 }
				 sf2 = calculslfundintent(PawnFundIntentListProtw.ProjectFinanceService,            // 资金类型
				    		intentDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							PawnFundIntentListProtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(payintentPeriod)),qs,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
							);
				/* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
							intentDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
			...
			sf =calculslfundintent(PawnFundIntentListProtw.ProjectLoadRoot,            // 资金类型
