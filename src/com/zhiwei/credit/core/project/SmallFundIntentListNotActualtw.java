...
public class SmallFundIntentListNotActualtw {
	/**
	 * 贷款贷出
	 */
	...
	public static List<SlFundIntent> getFundIntentDefaultList(String calcutype,
		...
		if (null ==isHaveLending ||isHaveLending.equals("yes")) { //像提前还款什么的都是不需要生成放款记录
			sf.setFundType(FundIntentListProtw.ProjectLoadOut);// 资金类型
		...
		sf3 = calculslfundintent(FundIntentListProtw.ProjectFinanceService,            // 资金类型
				...
				if(isInterestByOneTime==0){//不一次性首付利息
		    	   for (int i = 1; i <=payintentPeriod; i++) {
		    		 ...
		    		 /*  if(isPreposePayAccrual == 1){//前置付息
		    			   ...
		    			   intentDate=DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i-1);
		    		  // }
		    		   sf1 =calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadAccrual,            // 资金类型
		    				   intentDate,  //计划日期
		    		   ...
		    		   sf =calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadRoot,            // 资金类型
					   ...
					   sf2 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,managementConsultingOfRate).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP),i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay ,null,accrualType        // 收入金额
								);
	    		   ...
	    		   if(isPreposePayAccrual == 1){//前置付息
	    			 ...
	    			 if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
					   ...
					   sf2 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,managementConsultingOfRate).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(payintentPeriod)),1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					  /* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
				   ...
				   for (int i = 1; i <=payintentPeriod; i++) {
		    		   if(isPreposePayAccrual ==0 && i==payintentPeriod){//前置付息
						   ...
						   sf2 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectManagementConsulting,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
									SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,managementConsultingOfRate).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(payintentPeriod)),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						  /* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
				...
				if(isInterestByOneTime==0){//不一次性首付利息
		    	   for (int i = 1; i <=payintentPeriod; i++) {
					   ...
					   sf2 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,managementConsultingOfRate),i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   /*sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
	    		   ...
	    		   if(isPreposePayAccrual == 1){//前置付息
	    			 ...
	    			 if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
					   ...
					   sf2 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(payintentPeriod)),1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					  /* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
				   ...
				   for (int i = 1; i <=payintentPeriod; i++) {
		    		   ...
		    		   if(isPreposePayAccrual == 0 && i==payintentPeriod){
						   ...
						   sf2 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectManagementConsulting,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.addMonthsToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), payintentPeriod),-1),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
									SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(payintentPeriod)),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						  /* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.addMonthsToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), payintentPeriod),-1),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
				...
				if(isInterestByOneTime==0){//不一次性首付利息
		    	   for (int i = 1; i <=payintentPeriod; i++) {
					   ...
					   sf2 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(3)),i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					  /* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
	    		   ...
	    		   if(isPreposePayAccrual == 1){//前置付息
	    			 ...
	    			 list.add(sf1);
					   ...
					   sf2 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(payintentPeriod)).multiply(new BigDecimal(3)),1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay ,null,accrualType         // 收入金额
								);
					  /* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
				   ...
				   for (int i = 1; i <=payintentPeriod; i++) {
		    		   ...
		    		   if(isPreposePayAccrual == 0 && i==payintentPeriod){
						   ...
						   sf2 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectManagementConsulting,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.addMonthsToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i*3),-1),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
									SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(payintentPeriod)).multiply(new BigDecimal(3)),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						   /*sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.addMonthsToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i*3),-1),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
				...
				if(isInterestByOneTime==0){//不一次性支付利息
		    	   for (int i = 1; i <=payintentPeriod; i++) {
					   ...
					   sf2 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(12)),i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					  /* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
	    		   ...
	    		   if(isPreposePayAccrual == 1){//前置付息
	    			 ...
	    			 sf1 =calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadAccrual,            // 资金类型
		    				   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								projectMoney.multiply(accrual).multiply(new BigDecimal(12)).multiply(new BigDecimal(payintentPeriod)) ,1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType );
	    			 if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
	    			   list.add(sf1);
	    			 }	
					   sf2 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(12)).multiply(new BigDecimal(payintentPeriod)),1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   /*sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
				   ...
				   for (int i = 1; i <=payintentPeriod; i++) {
		    		   ...
		    		   if(isPreposePayAccrual == 0 && i==payintentPeriod){
						   ...
						   sf2 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectManagementConsulting,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.addMonthsToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i*12),-1),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
									SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(12)).multiply(new BigDecimal(payintentPeriod)),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						   /*sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.addMonthsToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i*12),-1),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
			...
			if(!isStartDatePay.equals("1")){//按实际放款日对日还款
				if(isInterestByOneTime==0){//不一次性首付利息
		    	   for (int i = 1; i <=payintentPeriod; i++) {
		    		   ...
		    		   sf =calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadRoot,            // 资金类型
		    				   DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i*dayOfEveryPeriod-1),  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								projectMoney.divide(new BigDecimal(payintentPeriod),2,BigDecimal.ROUND_UP) ,i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType );
		    		   if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
		    		   list.add(sf1);
					   ...
					   sf2 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,managementConsultingOfRate).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(dayOfEveryPeriod)),i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					  /* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
	    		   ...
	    		   if(isPreposePayAccrual == 1){//前置付息
	    			 ...
	    			 list.add(sf1);
					   ...
					   sf2 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectManagementConsulting,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,managementConsultingOfRate).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(dayOfEveryPeriod)).multiply(new BigDecimal(payintentPeriod)),1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   /*sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
				   ...
				   for (int i = 1; i <=payintentPeriod; i++) {
		    		   ...
		    		   if(isPreposePayAccrual ==0 && i==payintentPeriod){//前置付息
						   ...
						   sf2 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectManagementConsulting,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i*dayOfEveryPeriod),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
									SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,managementConsultingOfRate).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(dayOfEveryPeriod)).multiply(new BigDecimal(payintentPeriod)),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						  /* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
								   DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i*dayOfEveryPeriod),  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
	           ...
	           for (int i = 0; i < payintentPeriod; i++) {
	        	   Date intentDate=null;
					...
					/* }else{
					 ...
					 sf1 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadAccrual,            // 资金类型
							 intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,accrual).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP),i+1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType 	);        // 收入金额
					 if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
					 list.add(sf1);
					 }
					 sf2 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectManagementConsulting,            // 资金类型
					    		intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,managementConsultingOfRate).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP),i+1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					/* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
								intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
				...
				if(isPreposePayAccrual==1){
				 ...
				 sf1 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadAccrual,            // 资金类型
						 intentDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,accrual).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(payintentPeriod)),qs,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType 	);        // 收入金额
				 if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
				 list.add(sf1);
				 }
				 sf2 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectManagementConsulting,            // 资金类型
				    		intentDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							SmallFundIntentListNotActualtw.calculAccrualnew(projectMoney,managementConsultingOfRate).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(payintentPeriod)),qs,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
							);
				/* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
							intentDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
		...
		if (accrualType.equals("singleInterest") && payaccrualType.equals("owerPay")) {// 日尾
			...
			sf =calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadRoot,            // 资金类型
					DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), payintentPeriod*dayOfEveryPeriod-1),  //计划日期
					BigDecimal.valueOf(0),                                                // 支出金额
					projectMoney ,payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType 	);      // 收入金额
			list.add(sf);
		}
			 ...
			 if(isStartDatePay.equals("1")){ //固定在某日
				 ...
				 String firstdays=Integer.valueOf(DateUtil.getDaysBetweenDate( date1,sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1)))+1).toString();
				 	sf1 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadAccrual,            // 资金类型
							...
							BigDecimal.valueOf(0),                                                // 支出金额
							SmallFundIntentListNotActualtw.calculAccrualnew(preperiodsurplus,accrual).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(firstdays)),      // 收入金额
		    	...
		    	/*  BigDecimal financeServiceOfRate1=financeServiceOfRate.multiply(new BigDecimal(firstdays));
				 ...
				 sf1 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadAccrual,            // 资金类型
						 intentDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							SmallFundIntentListNotActualtw.calculAccrualnew(preperiodsurplus,accrual),
				 ...
				 sf1 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadAccrual,            // 资金类型
						 intDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							SmallFundIntentListNotActualtw.calculAccrualnew(preperiodsurplus,accrual).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(enddays)),      // 收入金额
				   ...
				   /* BigDecimal financeServiceOfRate2=financeServiceOfRate.multiply(new BigDecimal(enddays));
					...
					// BigDecimal financeServiceOfRateMoney=new BigDecimal(0);
						 ...
						 String firstdays=Integer.valueOf(DateUtil.getDaysBetweenDate( date1,sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1)))+1).toString();
						 accrualMoney=accrualMoney.add(SmallFundIntentListNotActualtw.calculAccrualnew(preperiodsurplus,accrual).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(firstdays)));
						 BigDecimal  managementConsultingOfRate1=managementConsultingOfRate.divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(firstdays));
						...
						// financeServiceOfRateMoney=financeServiceOfRateMoney.add(projectMoney.multiply(financeServiceOfRate1));
						 ...
						 for (int i = 2; i <payintentPeriod; i++) {
							 accrualMoney=accrualMoney.add(SmallFundIntentListNotActualtw.calculAccrualnew(preperiodsurplus,accrual));
							 managementConsultingOfRateMoney=managementConsultingOfRateMoney.add(projectMoney.multiply(managementConsultingOfRate));
						 ...
						 BigDecimal managementConsultingOfRate2=managementConsultingOfRate.divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(enddays));
						 BigDecimal financeServiceOfRate2=financeServiceOfRate.multiply(new BigDecimal(enddays));
						 accrualMoney=accrualMoney.add(SmallFundIntentListNotActualtw.calculAccrualnew(preperiodsurplus,accrual).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(enddays)));
		...
		if (accrualType.equals("sameprincipalandInterest") && payaccrualType.equals("monthPay")) {// 月尾
		     BigDecimal periodtimemoney=SmallFundIntentListNotActualtw.periodtimemoney(accrual.multiply(new BigDecimal(30)),projectMoney,payintentPeriod).divide(new BigDecimal(1),2,BigDecimal.ROUND_HALF_UP);
		     BigDecimal preperiodsurplus=projectMoney;
		     if(isStartDatePay.equals("1")){ //固定在某日
		    	 if(isInterestByOneTime==0){
			    	 String firstdays=Integer.valueOf(DateUtil.getDaysBetweenDate( date1,sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1)))+1).toString();
			    	 ...
			    	 sf1 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadAccrual,            // 资金类型
			    	  ...
			    	  BigDecimal firstperiodtimemoney=periodtimemoney.add(sf1.getIncomeMoney().subtract(SmallFundIntentListNotActualtw.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30))));
					    sf =calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadRoot,            // 资金类型
								DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1),  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
					...
					sf1 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadAccrual,            // 资金类型
							...
							BigDecimal.valueOf(0),                                                // 支出金额
							SmallFundIntentListNotActualtw.calculAccrualnew(preperiodsurplus,accrual),      // 收入金额
				    ...
				    sf =calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadRoot,            // 资金类型
							DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), i),  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							periodtimemoney.subtract(SmallFundIntentListNotActualtw.calculAccrualnew(preperiodsurplus,accrual)),        // 收入金额
							i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,String.valueOf(payintentPerioDate),accrualType
				    );
				    if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
						list.add(sf1);
					...
					*/
					...
					sf1 = calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadAccrual,            // 资金类型
			    	  ...
			    	  BigDecimal endperiodtimemoney=periodtimemoney.subtract((SmallFundIntentListNotActualtw.calculAccrualnew(preperiodsurplus,accrual)).subtract(sf1.getIncomeMoney()));
					    sf =calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadRoot,            // 资金类型
					    		DateUtil.addDaysToDate(DateUtil.addMonthsToDate(DateUtil.parseDate(date1), payintentPeriod), -1),  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
		    		...
		    		// BigDecimal financeServiceOfRateMoney=new BigDecimal(0);
			    	 ...
			    	 String firstdays=Integer.valueOf(DateUtil.getDaysBetweenDate( date1,sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1)))+1).toString();
			    	 accrualMoney=accrualMoney.add(SmallFundIntentListNotActualtw.calculAccrualnew(preperiodsurplus1,accrual).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(firstdays)));
			    	 BigDecimal managementConsultingOfRate1=managementConsultingOfRate.divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(firstdays));
			    	 managementConsultingOfRateMoney=managementConsultingOfRateMoney.add(projectMoney.multiply(managementConsultingOfRate1));
			    	/* BigDecimal financeServiceOfRate1=financeServiceOfRate.multiply(new BigDecimal(firstdays));
			    	 financeServiceOfRateMoney=financeServiceOfRateMoney.add(projectMoney.multiply(financeServiceOfRate1));*/
						...
						preperiodsurplus1=preperiodsurplus1.subtract(periodtimemoney.subtract(SmallFundIntentListNotActualtw.calculAccrualnew(preperiodsurplus1,accrual).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP)).multiply(new BigDecimal(firstdays)));
						
						
						
					for (int i = 2; i < payintentPeriod; i++) {
						accrualMoney=accrualMoney.add(SmallFundIntentListNotActualtw.calculAccrualnew(preperiodsurplus1,accrual));
						managementConsultingOfRateMoney=managementConsultingOfRateMoney.add(projectMoney.multiply(managementConsultingOfRate));
						//financeServiceOfRateMoney=financeServiceOfRateMoney.add(projectMoney.multiply(financeServiceOfRate).multiply(new BigDecimal(30)));
					
					preperiodsurplus1=preperiodsurplus1.subtract(periodtimemoney.subtract(SmallFundIntentListNotActualtw.calculAccrualnew(preperiodsurplus1,accrual)));
					}
					...
					String enddays=Integer.valueOf(DateUtil.getDaysBetweenDate( sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), payintentPeriod-1)),sdf.format(DateUtil.addDaysToDate(DateUtil.addMonthsToDate(DateUtil.parseDate(date1), payintentPeriod), -1)))).toString();
					accrualMoney=accrualMoney.add(SmallFundIntentListNotActualtw.calculAccrualnew(preperiodsurplus1,accrual).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(enddays)));
					BigDecimal managementConsultingOfRate2=managementConsultingOfRate.divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(enddays));
					managementConsultingOfRateMoney=managementConsultingOfRateMoney.add(projectMoney.multiply(managementConsultingOfRate2));
					BigDecimal financeServiceOfRate2=financeServiceOfRate.multiply(new BigDecimal(enddays));
					//financeServiceOfRateMoney=financeServiceOfRateMoney.add(projectMoney.multiply(financeServiceOfRate2));
					...
					if(isPreposePayAccrual==1){
					  ...
					  sf =calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadRoot,            // 资金类型
						...
						preperiodsurplus=preperiodsurplus.subtract(periodtimemoney.subtract(SmallFundIntentListNotActualtw.calculAccrualnew(preperiodsurplus,accrual)));
						for (int i = 2; i < payintentPeriod; i++) {
					    ...
					    sf =calculslfundintent(SmallFundIntentListNotActualtw.ProjectLoadRoot,            // 资金类型
					    		DateUtil.addDaysToDate(DateUtil.addMonthsToDate(DateUtil.parseDate(date1), payintentPeriod), -1),  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								periodtimemoney.subtract(SmallFundIntentListNotActualtw.calculAccrualnew(preperiodsurplus1,accrual).divide(new BigDecimal(30),10,BigDecimal.ROUND_HALF_UP).multiply(new BigDecimal(enddays))),        // 收入金额
								payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,String.valueOf(payintentPerioDate),accrualType
					    );
						list.add(sf);
		    	 ...
		    	 if(isInterestByOneTime==0){
		    		...
		    		// BigDecimal financeServiceOfRateMoney=new BigDecimal(0);
		    		 for (int i = 1; i < payintentPeriod+1; i++) {
						...
						preperiodsurplus=preperiodsurplus.subtract(periodtimemoney.subtract(FundIntentListPro3.calculAccrualnew(preperiodsurplus,accrual)));
					}
		    		 for (int i = 1; i < payintentPeriod+1; i++) {
