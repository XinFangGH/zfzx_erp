var add_button_o = new Ext.Button({
		    text : '添加租赁抵质押物材料项',
		    tooltip : '添加选中租赁抵质押物材料的租赁抵质押物材料项',
		    hidden:isGranted('_create_leaseFinance_dbdc')?false:true,
		    iconCls : 'addIcon'
}); 
var update_button_o = new Ext.Button({
		    text : '修改租赁抵质押物材料项',
		    tooltip : '修改选中的租赁抵质押物材料项',
		    hidden:isGranted('_edit_leaseFinance_dbdc')?false:true,
		    iconCls : 'updateIcon'
});
var delete_button_o = new Ext.Button({
			text : '删除租赁抵质押物材料项',
			tooltip : '从数据库中完全删除选中租赁抵质押物材料项',
			hidden:isGranted('_remove_leaseFinance_dbdc')?false:true,
			iconCls : 'deleteIcon'
});

var delete_button_lf1 = new Ext.Button({
			      
			text : '删除租赁抵质押物材料',
			tooltip : '从数据库中完全删除选中租赁抵质押物材料类别',
			iconCls : 'deleteIcon',
			scope : this,
			handler : function() {
				    ...
				    Ext.MessageBox.confirm('系统提示', '确认删除？删除后无法恢复!', function(btn) {
				    	       if (btn == 'no') {
				    	       	    return false;
										...
										Ext.Ajax.request({
											url : __ctxPath+ '/materials/deleteGlobalGuaranteeMaterialsExt.do',
											method : 'POST',
											params : {
												materialsId : id
											},
...
var update_button_lf1 = new Ext.Button({
				text : '修改租赁抵质押物材料',
				tooltip : '修改选中的租赁抵质押物材料类别',
				iconCls : 'updateIcon',
				 hidden:isGranted('_edit_leaseFinance_dbdc')?false:true,
				scope : this,
				handler : function() {
					...
					if (null == selected) {
						...
						var id = selected.get('materialsId'); // 获得当前行的ID值
						Ext.Ajax.request({
							url : __ctxPath+ '/materials/getGlobalGuaranteeMaterialsExt.do',
							method : 'POST',
							success : function(response, request) {
								...
								var _update_panel = new Ext.FormPanel({
									id : 'multupdatepanel',
									url : __ctxPath+ '/materials/saveGlobalGuaranteeMaterialsExt.do',
									labelAlign : 'right',
									buttonAlign : 'center',
							...
							xtype : 'textfield',
								...
								var UTwin = new Ext.Window({
											id : 'win',
											layout : 'fit',
											title : '编辑租赁抵质押物材料类型（根租赁抵质押物材料类型）',
											width : 500,
											height : 230,
											maximizable : true,
											modal : true,
...
var add_button_lf1 = new Ext.Button({
					text : '添加租赁抵质押物材料',
					scope : this,
					tooltip : '添加租赁抵质押物材料类型',
					iconCls : 'addIcon',
					hidden:isGranted('_create_leaseFinance_dbdc')?false:true,
					handler : function() {
					var formPanel = new Ext.FormPanel({
						...
						buttonAlign : 'center',
						url : __ctxPath
								+ '/materials/saveGlobalGuaranteeMaterialsExt.do',
						bodyStyle : 'padding:10px 25px 25px',
						labelWidth : 110,
						frame : true,
						waitMsgTarget : true,
...
var update_button_r = new Ext.Button({
						...
						handler : function() {
							...
							if (gselectid == '0') {
								Ext.ux.Toast.msg('操作提示', '请选择您要修改的租赁抵质押物材料项！');
								return;
							}
							Ext.Ajax.request({
								url : __ctxPath+ '/materials/getGlobalGuaranteeMaterialsExt.do',
								...
								success : function(response, request) {
																			...
																			.request(
																					{
																						...
																						success : function(response,request)
																					    {
																							...
																							if (objs.data == null) {
																								...
																								Ext.ux.Toast.msg('操作提示','请办输入正确的租赁抵质押物材料ID号!');
																								return;
																							} else {
									...
									var UTwin = new Ext.Window({
												id : 'win',
												layout : 'fit',
												title : '编辑数据租赁抵质押物材料项（子数据租赁抵质押物材料项）',
												width : 500,
												height : 300,
												maximizable : true,
												modal : true,
...
var delete_button_r = new Ext.Button({
						...
						handler : function() {
							...
							Ext.Ajax.request({
								url : __ctxPath
										+ '/materials/deleteGlobalGuaranteeMaterialsExt.do',
								method : 'POST',
								params : {
									materialsId : gselectid
								},
