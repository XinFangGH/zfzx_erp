...
LeaseFinanceMaterialsView = Ext.extend(Ext.Panel, {
	        projId:null,
	        gridPanel:null,
			...
			constructor : function(_cfg) {
				...
				LeaseFinanceMaterialsView.superclass.constructor.call(this, {
						    layout:'anchor',
		  					 anchor : '100%',
							 modal : true,
			...
			initUIComponents : function() {
				...
				var jsArr = [__ctxPath + '/js/creditFlow/leaseFinance/LeaseFinanceMaterialsForm.js'];
				...
				this.gridPanel = new xg.EditorGridPanel({
					   //listEnterpriseSlProcreditMaterials.do
				        store:  new Ext.data.GroupingStore({//listEnterpriseSlProcreditMaterials.do
	                        	proxy : new Ext.data.HttpProxy({url :__ctxPath + '/materials/listEnterpriseGlobalGuaranteeMaterialsDetail.do'}) ,
	                        	reader: reader,
                        ...
                        listeners : {
                		   'cellclick' : function(grid,rowIndex,columnIndex,e){
									...
									if("uploadOrDown"==fieldName){
										 ...
										 if(!this.ownerCt.isHiddenArchive && !this.ownerCt.isProjectInfoEdit){//项目保前归档节点并且不在项目信息编辑页面中
											 ...
											 loadData=function(size){    
												 ...
												 Ext.Ajax.request({
														url : __ctxPath + '/materials/updateDataNumGlobalGuaranteeMaterialsDetail.do',
														method : 'POST',
                            ...
                            'afteredit' : function(e) {
                            	    ...
                            	    Ext.Ajax.request({
										url : __ctxPath + '/materials/updateAfterGlobalGuaranteeMaterialsDetail.do',
										method : 'POST',
										success : function(response, request) {
	                                             e.record.commit();  
										},
			...
			createRs : function() {
				var recordId =  this.ownerCt.ownerCt.ownerCt.getCmpByName("procreditMortgage.id").getValue();
				if(typeof(recordId)=="undefined"||0==recordId){
					Ext.ux.Toast.msg('操作提示', '请先保存再上传文档');
					return;
				}
				new LeaseFinanceMaterialsForm({projectId : recordId,operateObj:this.gridPanel,isShow:false,businessType:this.businessType}).show();
			},
			updateRs : function(){
				var recordId =  this.ownerCt.ownerCt.ownerCt.getCmpByName("procreditMortgage.id").getValue();
				if(typeof(recordId)=="undefined"||0==recordId){
					Ext.ux.Toast.msg('操作提示', '请先保存再编辑文档');
					return;
				}
			    new LeaseFinanceMaterialsForm({projectId : recordId,operateObj:this.gridPanel,isShow:true,businessType:this.businessType}).show();
			},
			imageView :  function(){
