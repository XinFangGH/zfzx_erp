 ...
 * @createtime:
 */
LoanSettlementDocument = Ext.extend(Ext.Panel, {
	// 构造函数
	categoryId :0,
	contractId :0,
	isHiddenPanel : false,
	...
	constructor : function(_cfg) {
		...
		if (typeof(_cfg.LBTemplate) != 'undefined') {
		   ...
		   if(this.businessType == 'SmallLoan'&&this.LBTemplate=='LoanSettlement'){//业务类别，企业贷
				this.LBTemplate="LoanSettlement";//模板唯一标识
				this.titleText='贷款结清文档'
		   }else if(this.businessType == 'LeaseFinance'&&this.LBTemplate=='LoanSettlement'){
		   		this.LBTemplate="flLoanSettlement";//模板唯一标识
				this.titleText='贷款结清文档'
		   }
	    }
		...
		// 调用父类构造
		LoanSettlementDocument.superclass.constructor.call(this, {
	...
	createLetterAndBook : function(categoryId,contractId,titleText,projId,businessType,LBTemplate,cpanel){
		...
		Ext.Ajax.request({
			url : __ctxPath+'/contract/createAndDowloadDocumentProduceHelper.do',
			method : 'GET',
			success : function(response, request) {
				...
				if(obj.success==true){
					...
					pbar.getDialog().close();
					var conId=obj.data.id;
					cpanel.getStore().on("load",function(){
						if(conId == null || conId == 'undefined' || conId == ''){
							Ext.ux.Toast.msg('提示','操作失败，未找到相关文件！');
						}else{
							var pbar1 = Ext.MessageBox.wait('数据读取中','请等待',{
								interval:200,
						    	increment:15
							});
							window.open(__ctxPath+"/contract/lookUploadContractProduceHelper.do?conId="+conId,'_self');
							pbar1.getDialog().close();
						}
	...
	uploadLoanContractWin :function(id,contractId,fun,tId,businessType,title,projId){
		...
		/*if(id == null || id == 'undefined' || id == ''){
			...
			items:[new Ext.form.FormPanel({
				...
				url : __ctxPath+'/credit/document/uploadLoanSettlementDocument.do',
				monitorValid : true,
				labelAlign : 'right',
				buttonAlign : 'center',
