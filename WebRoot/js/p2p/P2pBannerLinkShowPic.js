 ...
 * @author Jiang Wanyu
 */
 var P2pBannerLinkShowPic = function(mark,readOnly,typeisfile,projectId,businessType){
    ...
    if(typeof(readOnly)!="undefined"){
 	...
 	var url=basepath();
 	url=url+"p2p/getwebPathListP2pFriendlink.do";
 	if(typeisfile !=null){
		 ...
		 returnUrl:function(extendname,webPath){
		  ...
		  download:function(v){
			...
			'<tpl for=".">',
			    ...
			    '</div>',
			    '<a href="'+p2pUrl+'/(values.webPath)" target="_blank"><span style="text-decoration:underline;cursor:pointer;height:15px">下载</span></a><span   if="this.imgOrDir(type) == false" class="">{filename}</span></div>',
	        '</tpl>',
	        '<div class="x-clear"></div>'
		,{
		 returnUrl:function(extendname,webPath){
