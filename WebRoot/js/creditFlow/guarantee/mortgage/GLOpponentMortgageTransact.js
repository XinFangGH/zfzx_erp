/**
 * @author:
 * @class GLOpponentMortgageTransact
 * @extends Ext.Panel
 * @description 反担保措施办理
 * @company 北京互融时代软件有限公司
 * @createtime:
 */
  var refreshMortgageGridStoregl = function(){
	Ext.getCmp('GLOpponentMortgageTransactId').getStore().reload();
}
GLOpponentMortgageTransact = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				GLOpponentMortgageTransact.superclass.constructor.call(this, {
							//id : 'GLOpponentMortgageTransact',
							layout:'anchor',
		  					anchor : '100%',
			...
			initUIComponents : function() {
				...
				this.editorGridPanel = new HT.EditorGridPanel({
					id : 'GLOpponentMortgageTransactId',
					tbar:this.topbar,
					border : false,
					//region : 'center',
					scope : this,
			...
			createMortgage : function() {
				//addMortgage(this.projectId,1,2,refreshMortgageGridStoregl,this.businessType);
				addMortgage(this.projectId,refreshMortgageGridStoregl,this.businessType,true);
			},
			// 编辑Rs
			...
			editMortgage : function() {
				...
				if (null == selected) {
					...
					if(typeId == 1){
							updateMortgageCar(mortgageId,refreshMortgageGridStoregl,businessType,true) ;
						}else if(typeId == 2){
							updateStockOwnerShip(mortgageId,refreshMortgageGridStoregl,businessType);
						}else if(typeId == 3){
							updateCompany(mortgageId,refreshMortgageGridStoregl,businessType);
						}else if(typeId == 4){
							updateDutyPerson(mortgageId,refreshMortgageGridStoregl,businessType);
						}else if(typeId == 5){
							updateMachineInfo(mortgageId,refreshMortgageGridStoregl,businessType);
						}else if(typeId == 6){
							updateProduct(mortgageId,refreshMortgageGridStoregl,businessType);
						}else if(typeId == 7){
							updateHouse(mortgageId,refreshMortgageGridStoregl,businessType);
						}else if(typeId == 8){
							updateOfficeBuilding(mortgageId,refreshMortgageGridStoregl,businessType);
						}else if(typeId == 9){
							updateHouseGround(mortgageId,refreshMortgageGridStoregl,businessType);
						}else if(typeId == 10){
							updateBusiness(mortgageId,refreshMortgageGridStoregl,businessType);
						}else if(typeId == 11){
							updateBusinessAndLive(mortgageId,refreshMortgageGridStoregl,businessType);
						}else if(typeId == 12){
							updateEducation(mortgageId,refreshMortgageGridStoregl,businessType);
						}else if(typeId == 13){
							updateIndustry(mortgageId,refreshMortgageGridStoregl,businessType);
						}else if(typeId == 14){
							updateDroit(mortgageId,refreshMortgageGridStoregl,businessType);
						}
				}
			},
			// 按ID删除记录
			...
			editMortgage : function() {
				...
				if (null == selected) {
					...
					var typeId = selected.get('typeid');
					if(typeId == 1){
							updateMortgageCar(mortgageId,refreshMortgageGridStoregl) ;
						}else if(typeId == 2){
							updateStockOwnerShip(mortgageId,refreshMortgageGridStoregl);
						}else if(typeId == 3){
							updateCompany(mortgageId,refreshMortgageGridStoregl);
						}else if(typeId == 4){
							updateDutyPerson(mortgageId,refreshMortgageGridStoregl);
						}else if(typeId == 5){
							updateMachineInfo(mortgageId,refreshMortgageGridStoregl);
						}else if(typeId == 6){
							updateProduct(mortgageId,refreshMortgageGridStoregl);
						}else if(typeId == 7){
							updateHouse(mortgageId,refreshMortgageGridStoregl);
						}else if(typeId == 8){
							updateOfficeBuilding(mortgageId,refreshMortgageGridStoregl);
						}else if(typeId == 9){
							updateHouseGround(mortgageId,refreshMortgageGridStoregl);
						}else if(typeId == 10){
							updateBusiness(mortgageId,refreshMortgageGridStoregl);
						}else if(typeId == 11){
							updateBusinessAndLive(mortgageId,refreshMortgageGridStoregl);
						}else if(typeId == 12){
							updateEducation(mortgageId,refreshMortgageGridStoregl);
						}else if(typeId == 13){
							updateIndustry(mortgageId,refreshMortgageGridStoregl);
						}else if(typeId == 14){
							updateDroit(mortgageId,refreshMortgageGridStoregl);
						}
				}
			},*/
			//查看详情
