...
public class BpBusinessOrProDaoImpl extends BaseDaoImpl<BpBusinessOrPro> implements BpBusinessOrProDao{

	public BpBusinessOrProDaoImpl() {
		super(BpBusinessOrPro.class);
	}

	@Override
	public BpBusinessOrPro getBpBusinessOrProByMoneyPlanId(Long moneyPlanId) {
		String hql = "from BpBusinessOrPro as bop where bop.moneyPlanId = ?";
		List list = new ArrayList();
		list = super.findByHql(hql, new Object[]{moneyPlanId});
		if(list!=null&&list.size()!=0){
			return (BpBusinessOrPro) list.get(0);
		}
		return null;
	}
	...
	//	System.out.println("-->"+hql.toString());
		if(null!=pb){
			return this.getSession().createSQLQuery(hql.toString())
					.addScalar("borProId", Hibernate.LONG)
			        ...
			        .setResultTransformer(Transformers.aliasToBean(BpBusinessOrPro.class)).setFirstResult(pb.getFirstResult()).setMaxResults(pb.getPageSize()).list();
		}
		else{
			return this.getSession().createSQLQuery(hql.toString())
				.addScalar("porProId", Hibernate.LONG)
					...
					.addScalar("endDate", Hibernate.DATE)
					.addScalar("continuationMoney", Hibernate.BIG_DECIMAL)
					.setResultTransformer(Transformers.aliasToBean(BpBusinessOrPro.class)).list();
