 ...
 * 、
 */
PawnUploads = Ext.extend(Ext.Panel, {
	isDisabled : false,
	// 构造函数
	...
	initUIComponents : function() {
		...
		var checkSubmitColumn = new Ext.grid.CheckColumn({
			hidden : this.issubmitHidden==false?false:true,
			editable : this.issubmitEdit,
			header : '是否提交',
			dataIndex : 'isSubmit',
			fixed : true,
			width : 74
		...
		this.datefield=new Ext.form.DateField({
			format : 'Y-m-d',
			readOnly:this.issubmitHidden,
			allowBlank : false
		})
		this.datefield1=new Ext.form.DateField({
			format : 'Y-m-d',
			readOnly:this.isgdHidden,
			allowBlank : false
		})
		this.render_UploadsInfo = Ext.data.Record.create([{name : 'fileid'},{name : 'filename'}, {name : 'filepath'}, {name : 'extendname'}, {name : 'filesize'}, {name : 'createtime'}, {name : 'contentType'}, {name : 'webPath'},{name : 'projId'},{name : 'isArchiveConfirm'},{name : 'archiveConfirmRemark'},{name : 'archiveConfirmDate'},{name : 'isSubmit'},{name : 'submitDate'},{name : 'submitRemarks'}]);
		...
		this.grid_UploadsPanel = new HT.EditorGridPanel({
			...
			plugins : [checkColumn,checkSubmitColumn],
			...
			listeners:{
				...
				afteredit : function(e) {
					...
					if (e.originalValue != e.value) {//编辑行数据发生改变
						...
						if(e.field == 'archiveConfirmDate') {
							args = {'fileinfo.archiveConfirmDate': value,'fileinfo.fileid': id};
						}
						if(e.field == 'submitRemarks') {
							args = {'fileinfo.submitRemarks': value,'fileinfo.fileid': id};
						}
						if(e.field == 'submitDate') {
							args = {'fileinfo.submitDate': value,'fileinfo.fileid': id};
						}
						if(e.field == 'isSubmit') {
							args = {'fileinfo.isSubmit': value,'fileinfo.fileid': id};
						}
						Ext.Ajax.request({
	...
	deleteFileCommon : function(fileid,grid){
		...
		reUploadForThisPanel : function(titleName,typeisfile,projId,businessType,displayDel,fileid,mark,templettype,panel,setname){
			...
			reUploadReportJSExt('上传/下载"'+titleName+'"',typeisfile,mark,2,null,null,reloadjStore,projId,businessType,fileid,setname);
		}
