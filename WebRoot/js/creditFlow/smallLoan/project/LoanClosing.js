 ...
 * @createtime:
 */
LoanClosing = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		LoanClosing.superclass.constructor.call(this, {
			id : 'LoanClosing',
			title : '贷款结清',
			region : 'center',
			layout : 'border',
			iconCls :'btn-tree-team11',
	...
	initUIComponents : function() {
		...
		this.topbar = new Ext.Toolbar({
			items : [ {
						...
						xtype : 'button',
						scope : this,
						//hidden : isGranted('_loanClosing_flow')? false: true,
						handler : this.detailPro
					}, new Ext.Toolbar.Separator({
						//hidden : isGranted('_loanClosing_projectInfo')? false: true
					}),{
						iconCls : 'btn-detail',
						text : '查看贷款详情',
						xtype : 'button',
		...
		this.gridPanel = new HT.GridPanel( {
			...
			notmask :true,
			tbar:this.topbar,
			url : __ctxPath + "/flow/getByProcessNameNodeKeyOfCommonTask.do?processName=smallLoanCommonFlow&flowNodeKey=slcfLoanWindUpACase&searchType=byUserId",
			fields : [ {
				name : 'task.taskId',
				type : 'int'
			}, 'task.dueDate',
			...
			'vSmallloanProject.loanStartDate',
			'vSmallloanProject.expectedRepaymentDate'
			],
			columns : [ {
				header : 'id',
				...
				header : '贷款开始时间',
				dataIndex : 'vSmallloanProject.loanStartDate'
			}, {
				header : '贷款结束时间',
				dataIndex : 'vSmallloanProject.expectedRepaymentDate'
			}, {
				header : '提交时间',
