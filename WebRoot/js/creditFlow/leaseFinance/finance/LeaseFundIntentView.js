...
LeaseFundIntentView = Ext.extend(Ext.Panel, {
	name:"LeaseFundIntentView",
	infoObj : null,
	isHiddenAddBtn : true,
	isHiddenDelBtn : true,
	isHiddenCanBtn : true,
	...
	constructor : function(_cfg) {
		...
		// 调用父类构造
		LeaseFundIntentView.superclass.constructor.call(this, {
			name : "LeaseFundIntentView_panel",
	...
	initUIComponents : function() {
		...
		this.topbar = new Ext.Toolbar({
			items : [{
				...
				text : '导出租赁定价表',
				xtype : 'button',
				hidden : this.isHiddenFinanceExcel,
				scope : this,
				handler : function() {
					this.toFinanceExcel();
				}
...
//								}
	...
	createRs : function() {
		...
		var payintentPeriod = (typeof(this.object)!='undefined'?this.object.getCmpByName('flLeaseFinanceProject.payintentPeriod').getValue():this.payintentPeriod);
		   ...
		   this.oneopenliushuiwin();
			...
			autocreate : function() { 
				var businessType=this.businessType;
					var gridPanel1=this.gridPanel
					 ...
					 var payintentPeriod=null;
					  ...
					  accrualtype=this.object.getCmpByName("flLeaseFinanceProject.accrualtype").getValue();
					   payaccrualType=this.object.getCmpByName("flLeaseFinanceProject.payaccrualType").getValue();
					    projectMoney=this.object.getCmpByName("flLeaseFinanceProject.projectMoney").getValue();
					   isStartDatePay=this.object.getCmpByName("flLeaseFinanceProject.isStartDatePay").getValue();
					  
					   startDate=this.object.getCmpByName("flLeaseFinanceProject.startDate").getValue();
					   payintentPerioDate=this.object.getCmpByName("flLeaseFinanceProject.payintentPerioDate").getValue();
					   payintentPeriod=this.object.getCmpByName("flLeaseFinanceProject.payintentPeriod").getValue();
					    intentDate=this.object.getCmpByName("flLeaseFinanceProject.intentDate").getValue();
					     dayOfEveryPeriod=this.object.getCmpByName("flLeaseFinanceProject.dayOfEveryPeriod").getValue();
					   leaseDepositRate=this.object.getCmpByName("flLeaseFinanceProject.leaseDepositRate").getValue();
					    leaseDepositMoney=this.object.getCmpByName("flLeaseFinanceProject.leaseDepositMoney").getValue();
					    rentalFeeRate=this.object.getCmpByName("flLeaseFinanceProject.rentalFeeRate").getValue();
					     rentalFeeMoney=this.object.getCmpByName("flLeaseFinanceProject.rentalFeeMoney").getValue();
					     leaseRetentionFeeRate=this.object.getCmpByName('flLeaseFinanceProject.leaseRetentionFeeRate').getValue();
					     leaseRetentionFeeMoney=this.object.getCmpByName('flLeaseFinanceProject.leaseRetentionFeeMoney').getValue();
					     rentalRate=this.object.getCmpByName('flLeaseFinanceProject.rentalRate').getValue();
					     rentalMoney=this.object.getCmpByName('flLeaseFinanceProject.rentalMoney').getValue();
					     feePayaccrualType=this.object.getCmpByName('flLeaseFinanceProject.feePayaccrualType').getValue();
					    
					 	eachRentalReceivable=this.object.getCmpByName("flLeaseFinanceProject.eachRentalReceivable").getValue();
					     ...
					     message="还款方式";
					   	 ...
					   	 message="还款周期";
							   ...
							   if(startDate=="" || startDate==null){
							    ...
							    if(leaseDepositRate=="" || leaseDepositRate==null){
							    	message="租赁保证金费率";
							   }
							   if(rentalFeeRate=="" || rentalFeeRate==null){
							    	message="租赁手续费率";
							   }
							   if(leaseRetentionFeeRate=="" || leaseRetentionFeeRate==null){
							    	message="租赁物件留购费率";
							   }
			 ...
			 if(message !=""){   
						...
						var params1 ={
							projectId : projId,
						    'flLeaseFinanceProject.accrualtype': accrualtype,
						    'flLeaseFinanceProject.payaccrualType': payaccrualType,
						    'flLeaseFinanceProject.projectMoney': projectMoney,
						    'flLeaseFinanceProject.startDate': startDate,
						    'flLeaseFinanceProject.intentDate': intentDate,
						    'flLeaseFinanceProject.isStartDatePay': isStartDatePay,
						    'flLeaseFinanceProject.payintentPerioDate': payintentPerioDate,
						     'flLeaseFinanceProject.payintentPeriod': payintentPeriod,
						         'flLeaseFinanceProject.dayOfEveryPeriod': dayOfEveryPeriod,
						     'flLeaseFinanceProject.leaseDepositRate': leaseDepositRate,
						     'flLeaseFinanceProject.leaseDepositMoney': leaseDepositMoney,
						      'flLeaseFinanceProject.rentalFeeRate': rentalFeeRate,
						       'flLeaseFinanceProject.rentalFeeMoney': rentalFeeMoney,
						       
						         'flLeaseFinanceProject.leaseRetentionFeeRate':leaseRetentionFeeRate,
						          'flLeaseFinanceProject.leaseRetentionFeeMoney': leaseRetentionFeeMoney,
						         'flLeaseFinanceProject.rentalRate':rentalRate,
						         'flLeaseFinanceProject.rentalMoney':rentalMoney,
						         'flLeaseFinanceProject.feePayaccrualType':feePayaccrualType,
						         'flLeaseFinanceProject.eachRentalReceivable':eachRentalReceivable,
					...
					toFinanceExcel:function(){
						var projectId = this.projectId;
						var businessType=this.businessType;
						Ext.Ajax.request({
							url : __ctxPath + "/creditFlow/leaseFinance/project/getFlLeaseFinanceProject.do",
							method : 'POST',
							scope:this,
							success : function(response, request) {
								var obj = Ext.util.JSON.decode(response.responseText);
								if(obj.data.accrualtype=='matchingRepayment'){
									window.open( __ctxPath + "/creditFlow/finance/leaseDownloadSlFundIntent.do?projectId="+projectId+"&businessType="+businessType,'_blank');
								}else{
									Ext.ux.Toast.msg('操作信息','目前只支持还款方式为等额还款的导出');
								}
							},
					...
					/*	var payaccrualType=this.object.getCmpByName("flLeaseFinanceProject.payaccrualType").getValue();
						var payintentPeriod=this.object.getCmpByName("flLeaseFinanceProject.payintentPeriod").getValue();
					    var dayOfEveryPeriod=this.object.getCmpByName("flLeaseFinanceProject.dayOfEveryPeriod").getValue();
					    var leaseDepositRate=this.object.getCmpByName("flLeaseFinanceProject.leaseDepositRate").getValue();
					    var leaseDepositMoney=this.object.getCmpByName("flLeaseFinanceProject.leaseDepositMoney").getValue();
					    var rentalFeeRate=this.object.getCmpByName("flLeaseFinanceProject.rentalFeeRate").getValue();
					    var rentalFeeMoney=this.object.getCmpByName("flLeaseFinanceProject.rentalFeeMoney").getValue();
					    var leaseRetentionFeeRate=this.object.getCmpByName('flLeaseFinanceProject.leaseRetentionFeeRate').getValue();
					    var leaseRetentionFeeMoney=this.object.getCmpByName('flLeaseFinanceProject.leaseRetentionFeeMoney').getValue();
					    var rentalRate=this.object.getCmpByName('flLeaseFinanceProject.rentalRate').getValue();
					    var rentalMoney=this.object.getCmpByName('flLeaseFinanceProject.rentalMoney').getValue();
					    var feePayaccrualType=this.object.getCmpByName('flLeaseFinanceProject.feePayaccrualType').getValue();
					 	var eachRentalReceivable=this.object.getCmpByName("flLeaseFinanceProject.eachRentalReceivable").getValue();
					 	var projectMoney=this.object.getCmpByName("flLeaseFinanceProject.projectMoney").getValue();
					 	var allMoney=this.object.getCmpByName("flLeaseFinanceProject.allMoney").getValue();
					 	var connotationRate=this.object.getCmpByName('flLeaseFinanceProject.connotationRate').getValue();
					 	var accrualtype=this.object.getCmpByName('flLeaseFinanceProject.accrualtype').getValue();
					 	var startDate=this.object.getCmpByName('flLeaseFinanceProject.startDate').getValue();
					 	startDate=Ext.util.Format.date(startDate, 'Y/m/d')
					 	var intentDate=this.object.getCmpByName('flLeaseFinanceProject.intentDate').getValue();
					 	intentDate=Ext.util.Format.date(intentDate, 'Y/m/d')
					 	var vDatas=this.getGridData();*/
					 	//if(this.accrualtype=='matchingRepayment'){
