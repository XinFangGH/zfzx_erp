...
function addTemplate(node ,documentTreePanel){
	...
	if(node.attributes.onlymark =='ReportTemplate'||node.attributes.onlymark =='RiskReportTemplate'||node.attributes.onlymark =='GuaranteeRiskReport'||node.attributes.onlymark =='GuaranteeReport'||node.attributes.onlymark =='zijing'){
		var hidLeaf = new Ext.form.Hidden({
			name : 'documentTemplet.leaf',
			value : 1
		})
		...
		if(node.attributes.onlymark =='XEDZ'){
			var objFieldOnly = new Ext.form.TextField({
				fieldLabel : '唯一标识KEY',
				name : 'documentTemplet.onlymark',
				allowBlank : false,
		...
		if(node.attributes.isChild =='0'&& node.attributes.onlymark !='XEDZ'){
			var hidLeaf = new Ext.form.Hidden({
				name : 'documentTemplet.leaf',
				value : 0
			})
		...
		if(node.attributes.isChild =='1'&& node.attributes.onlymark !='XEDZZQ'){
			var hidLeaf = new Ext.form.Hidden({
				name : 'documentTemplet.leaf',
				value : 1
			})
