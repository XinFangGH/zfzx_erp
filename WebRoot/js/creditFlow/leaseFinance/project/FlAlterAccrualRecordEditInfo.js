 ...
 * @author
 * @createtime
 * @class FlAlterAccrualRecordEditInfo
 * @extends Ext.Window
 * @description 融资租赁---利率变更详情
 * @company 智维软件
 */
FlAlterAccrualRecordEditInfo = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		this.initComponents();
		// 调用父类构造
		FlAlterAccrualRecordEditInfo.superclass.constructor.call(this, {
			autoScroll:true,
			...
			tbar :new Ext.Toolbar({
				...
				items : ['->',{
				iconCls : 'btn-save',
				hidden : !(isGranted('_hiddenLilvSaveTbar_fl'+this.projectStatus+this.bmStatus)||isGranted('_seehiddenLilvSaveTbar_fl'+this.projectStatus+this.bmStatus)),
				text : '保存',
				xtype : 'button',
				scope : this,
				handler : this.save
	...
	initComponents:function(){
		...
		var DeferApplyInfoPanel = new FinanceAlterAccrualPanel({
			...
			payintentPeriod : 4,
			isAllReadOnly:!(isGranted('_hiddenLilvSaveTbar_fl'+this.projectStatus+this.bmStatus)||isGranted('_seehiddenLilvSaveTbar_fl'+this.projectStatus+this.bmStatus)),
			idDefinition:'LeaseFinance',
			projectId : projectId,
		...
		this.alterAccrualSlFundIntent = new AlterAccrualSlFundIntent({
			...
			projId : projectId,
			titleText : '放款收息表',
			isHidden : !(isGranted('_lilvOpraFksxb_fl'+this.projectStatus+this.bmStatus)||isGranted('_seelilvOpraFksxb_fl'+this.projectStatus+this.bmStatus)),
			...
			items : [{
        		...
        		items : [
					new LetterAndBookView({
						...
						isHidden:!(isGranted('_lilvSpbsc_fl'+this.projectStatus+this.bmStatus)||isGranted('_seelilvSpbsc_fl'+this.projectStatus+this.bmStatus))
					})
				]
			}]
		})
		this.loadData({
			url : __ctxPath + '/smallLoan/finance/flGetInfoLoanedSlAlterAccrualRecord.do?projectId='+projectId+'&type='+this.businessType+'&loanedTypeId='+slAlteraccrualRecordId+'&loanedTypeKey=slAlteraccrual',
			method : "POST",
