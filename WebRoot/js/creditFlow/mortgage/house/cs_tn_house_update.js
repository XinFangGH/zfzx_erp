var updateHouse = function(mortgageid,refreshMortgageGridStore,businessType){
		...
		var window_updateHouse = new Ext.Window({
			...
			listeners : {
				'beforeclose' : function(){
					if(panel_updateHouse.getForm().isDirty()){
						Ext.Msg.confirm('操作提示','数据被修改过,是否保存?',function(btn){
							if(btn=='yes'){
								panel_updateHouse.getTopToolbar().items.itemAt(0).handler.call() ;
							}else{
								panel_updateHouse.getForm().reset() ;
								window_updateHouse.destroy() ;
							}
						}) ;
						return false ;
					}
