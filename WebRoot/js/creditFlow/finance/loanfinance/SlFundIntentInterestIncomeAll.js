	...
	initUIComponents : function() {
		...
		this.topbar = new Ext.Toolbar({
					items : [{
						...
						xtype : 'button',
						scope : this,
						hidden : isGranted('_liushui_f_interestIncome_'
					...
					, {
						...
						xtype : 'button',
						scope : this,
						hidden : isGranted('_liushuisee_f_interestIncome_'
								+ this.businessType) ? false : true,
						...
						xtype : 'button',
						scope : this,
						hidden : isGranted('_ping_f_interestIncome_'
								+ this.businessType) ? false : true,
						...
						xtype : 'button',
						scope : this,
						hidden : isGranted('_fa_f_interestIncome_'
								+ this.businessType) ? false : true,
						...
						xtype : 'button',
						scope : this,
						hidden : isGranted('_uploaddownpz_f_interestIncome_'
								+ this.businessType) ? false : true,
						handler : this.upload
		...
		this.gridPanel = new HT.GridPanel({
			...
			loadMask : true,
			url : __ctxPath
					+ "/creditFlow/finance/listbyLedgerSlFundIntent.do?businessType="
					+ this.businessType + this.getfundType() + "&typetab=all",
			fields : [{
						name : 'fundIntentId',
						type : 'int'
					}, 'projectName', 'projectNumber', 'incomeMoney',
					'fundTypeName', 'intentDate', 'payMoney', 'payInMoney',
					'factDate', 'fundType', 'afterMoney', 'notMoney',
					'flatMoney', 'isOverdue', 'overdueRate', 'accrualMoney',
					'status', 'remark', 'businessType', 'projectId', 'tabType',
					'isInitialorId', 'orgName', 'companyId', 'graceDay',
					'continueDay', 'faxiAfterMoney','payintentPeriod', 'interestStarTime',
					'interestEndTime','payintentPeriod','ids','bidPlanId'],
			columns : [{
						header : 'fundIntentId',
...
/*,
	...
	planformToInvestor : function(record) {
		...
		if (s <= 0) {
			...
			var fundIntentId = record.data.fundIntentId;
			var url = __ctxPath
					+ "/creditFlow/finance/planformToInvestorSlFundIntent.do?fundIntentId="
					+ fundIntentId;
	...
	borrowerToInvestor : function(record) {
		...
		if (s <= 0) {
			...
			var url = __ctxPath
					+ "/creditFlow/finance/borrowerToInvestorSlFundIntent.do?fundIntentId="
					+ fundIntentId;
		...
		if (s <= 0) {
			...
			var bidPlanId=record.data.bidPlanId;
				new SlRepaymentView({
							fundIntentId : fundIntentId,
							fundType : fundType,// 利息
							projectId : projectId,
	...
	cellClick : function(grid, rowIndex, columnIndex, e) {
		if (17 == columnIndex) {
			...
			var accrualMoney = grid.getStore().getAt(rowIndex)
					.get('accrualMoney');
			var fundIntentId = grid.getStore().getAt(rowIndex)
					.get('fundIntentId');
			var businessType = this.businessType;
			if (accrualMoney != 0) {
				new SlPunishInterestView({
	...
	exportExcel:function(){
		...
		var propertiesObj=this.getCmpByName("projectProperties");
		if(propertiesObj){
			projectProperties=propertiesObj.getValue();
		}
		
		var fundType="";
		if(this.businessType=="SmallLoan"){
			fundType="('loanInterest')";
		}else if(this.businessType=="consultationMoney"){
			fundType="('consultationMoney')";
		}else if(this.businessType=="serviceMoney"){
