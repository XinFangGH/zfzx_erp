 ...
 * @author Jiang Wanyu
 */
 var BpCustMemberPicView = function(mark,readOnly,typeisfile,projectId,businessType){
    ...
    if(typeof(readOnly)!="undefined"){
 	...
 	var url=basepath();
 	url=url+"p2p/getPicPathWebFinanceApplyUploads.do";
 	if(typeisfile !=null){
 	...
 	if(!isReadOnly){
			...
			'<tpl for=".">',
			    ...
			    '</div>',
			    '<span onclick="download1({[this.download(values.webPath)]})" style="text-decoration:underline;cursor:pointer;height:15px">下载</span>' +
//			    '<span onclick="removeImg1({[this.removeImg(values.fileid)]})" style="text-decoration:underline;cursor:pointer;height:15px">删除</span>' +
		...
		,{
			returnwebPath:function(webPath){
				return webPath;
			},
			removeImg:function(fileid){//返回移除图片参数
				return fileid;
			},
		 returnUrl:function(extendname,webPath){
//		  alert(__p2pPath+ webPath);
            			...
            			var i=0;
			               ...
			               ds.each(function(record){
			                //img+=record.get('webPath')+',';
			               		 img[i]=__p2pPath+record.get('webPath');
			                	i++;
			               });
			               var indx = img[index];
			               		 ...
			               		 if(indx.substring(last,indx.length)!='.jpg'&&indx.substring(last,indx.length)!='.png'&&indx.substring(last,indx.length)!='.gif'){
			               			showPic(__p2pPath+jStore.getAt(index).get('webPath'),'图片查看-'+jStore.getAt(index).get('filename'),jStore.getAt(index).get('extendname'),jStore.getAt(index).get('fileAttachID'));
...
//    window.open(__ctxPath+"/creditFlow/fileUploads/DownLoadFileForm.do?id="+v,'_blank');
//};
function download1(v){
    window.open(__ctxPath+"/p2p/downloadFriendLinkWebFinanceApplyUploads.do?picURL="+v,'_blank');
};
//function removeImg1(v){
//    window.open(__ctxPath+"/creditFlow/fileUploads/DownLoadnewFileForm.do?id="+v,'_blank');
//};
