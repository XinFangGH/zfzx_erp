 ...
 * @company 智维软件
 */
PlManageMoneyProducebuy = Ext.extend(Ext.Window, {
	// 构造函数
	constructor : function(_cfg) {
		Ext.applyIf(this, _cfg);
		// 必须先初始化组件
		this.initUIComponents();
		PlManageMoneyProducebuy.superclass.constructor.call(this, {
					id : 'PlManageMoneyProducebuy',
					layout : 'fit',
					border : false,
					items : [
					this.formPanel],
					...
					//		id : 'plManageMoneyPlanBuyinfo.investPersonName',
						...
						items : [{
									...
									listeners : {
										scope : this,
										'blur' : function(bm){
											var startMoney=eval(this.getCmpByName('startMoney1').getValue());//投资起点金额
											var limitMoney=eval(this.getCmpByName('limitMoney1').getValue());//投资上限
											if(bm.getValue()<startMoney){
												Ext.ux.Toast.msg('操作信息', '购买金额必须大于起点金额!');
												bm.setValue();
											}
											if(bm.getValue()>limitMoney){
												Ext.ux.Toast.msg('操作信息', '购买金额必须小于上限金额!');
												bm.setValue();
											}
										}
									}
	...
	save : function() {
		...
		$postForm({
			...
			scope : this,
			url : __ctxPath
					+ '/creditFlow/financingAgency/saveProdeucePlManageMoneyPlanBuyinfo.do',
			callback : function(fp, action) {
				var object = Ext.util.JSON.decode(action.response.responseText);
						if(object.moneytoobig==true){
