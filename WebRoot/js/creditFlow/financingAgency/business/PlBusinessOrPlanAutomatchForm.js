 ...
 * @company 智维软件
 */
PlBusinessOrPlanAutomatchForm = Ext.extend(Ext.Window, {
	// 构造函数
	constructor : function(_cfg) {
		Ext.applyIf(this, _cfg);
		// 必须先初始化组件
		this.initUIComponents();
		PlBusinessOrPlanAutomatchForm.superclass.constructor.call(this, {
					id : 'PlBusinessOrPlanAutomatchForm',
					layout : 'fit',
	...
	initUIComponents : function() {
			...
			items : [{
						...
						items : [{
							...
							listeners : {
								...
								'change' : function(nf) {
									var totalMoney = Ext
											.getCmp('loanProMoney').getValue();
									var currMoney = Ext.getCmp('orMoney')
											.getValue();
									var residueMoney = Ext
											.getCmp('residueMoney').getValue();

									if (currMoney > residueMoney) {
										alert("本次转让金额不能大于剩余金额！");
										currMoney = residueMoney;
									}
									Ext.getCmp('orMoneyScale')
											.setValue(currMoney / totalMoney
													* 100);
									Ext.getCmp('residueMoney')
											.setValue(residueMoney - currMoney);
	    ...
	    Ext.Ajax.request({
			url : __ctxPath
						+ '/creditFlow/financingAgency/business/getBidInfoBpBusinessOrPro.do?borProId='
						+ this.borProId,
			method:"post",
