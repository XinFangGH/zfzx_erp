...
public class LeaseFinanceFundIntentListProtw {
	/**
	 * 租赁成本发放
	 */
	...
	public static List<SlFundIntent> getFundIntentDefaultList(String calcutype,
		...
		if (null ==isHaveLending ||isHaveLending.equals("yes")) { //像提前还款什么的都是不需要生成放款记录
			sf.setFundType(LeaseFinanceFundIntentListProtw.ProjectRentalCostsPaid);// 资金类型
		...
		/*int alldays=DateUtil.getDaysBetweenDate(date1, date2)+1;
		sf3 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectFinanceService,            // 资金类型
		    DateUtil.parseDate(date1,"yyyy-MM-dd"),  //计划日期
			BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
				...
				if(isInterestByOneTime==0){//不一次性首付利息
		    	   for (int i = 1; i <=payintentPeriod; i++) {
		    		 ...
		    		 /*  if(isPreposePayAccrual == 1){//前置付息
		    			   ...
		    			   intentDate=DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i-1);
		    		  // }
		    		   sf1 =calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectLeaseFeeCharging,            // 资金类型
		    				   intentDate,  //计划日期
		    		   ...
		    		   sf =calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectLeasePrincipalRepayment,            // 资金类型
		    		   ...
		    		   if(feePayaccrualType.equals("withRentalFee")){
						   sf2 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectRentalFeeCharging,            // 资金类型
								   intentDate,  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
									LeaseFinanceFundIntentListProtw.calculAccrualnew(projectMoney,managementConsultingOfRate),i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay ,null,accrualType        // 收入金额
									);
	    		   ...
	    		   if(isPreposePayAccrual == 1){//前置付息
	    			 ...
	    			 if(feePayaccrualType.equals("withRentalFee")){
						   sf2 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectRentalFeeCharging,            // 资金类型
								   intentDate,  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
									LeaseFinanceFundIntentListProtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(payintentPeriod)),1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						  /* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
								   intentDate,  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
		    		   ...
		    		   if(isPreposePayAccrual ==0 && i==payintentPeriod){//前置付息
		    			   ...
		    			   if(feePayaccrualType.equals("withRentalFee")){
							   sf2 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectRentalFeeCharging,            // 资金类型
									   DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i),  //计划日期
										BigDecimal.valueOf(0),                                                // 支出金额
										LeaseFinanceFundIntentListProtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(payintentPeriod)),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
										);
							  /* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
									   DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i),  //计划日期
										BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
		    	   ...
		    	   for (int i = 1; i <=payintentPeriod; i++) {
		    		   ...
		    		   if(feePayaccrualType.equals("withRentalFee")){
						   sf2 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectRentalFeeCharging,            // 资金类型
								   intentDate,  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
									LeaseFinanceFundIntentListProtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(30)),i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						   /*sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
								   intentDate,  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
	    		   ...
	    		   if(isPreposePayAccrual == 1){//前置付息
	    			 ...
	    			 if(feePayaccrualType.equals("withRentalFee")){
						   sf2 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectRentalFeeCharging,            // 资金类型
								   intentDate,  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
									LeaseFinanceFundIntentListProtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)),1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						  /* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
								   intentDate,  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
		    		   ...
		    		   if(isPreposePayAccrual == 0 && i==payintentPeriod){
		    			   ...
		    			   if(feePayaccrualType.equals("withRentalFee")){
							   sf2 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectRentalFeeCharging,            // 资金类型
									   DateUtil.addMonthsToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), payintentPeriod),  //计划日期
										BigDecimal.valueOf(0),                                                // 支出金额
										LeaseFinanceFundIntentListProtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
										);
							  /* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
		    	   ...
		    	   for (int i = 1; i <=payintentPeriod; i++) {
		    		   ...
		    		   if(feePayaccrualType.equals("withRentalFee")){
						   sf2 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectRentalFeeCharging,            // 资金类型
								   intentDate,  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
									LeaseFinanceFundIntentListProtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(3)),i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						  /* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
								   intentDate,  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
	    		   ...
	    		   if(isPreposePayAccrual == 1){//前置付息
	    			 ...
	    			 if(feePayaccrualType.equals("withRentalFee")){
						   sf2 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectRentalFeeCharging,            // 资金类型
								   intentDate,  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
									LeaseFinanceFundIntentListProtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)).multiply(new BigDecimal(3)),1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay ,null,accrualType         // 收入金额
									);
						  /* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
								   intentDate,  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
		    		   ...
		    		   if(isPreposePayAccrual == 0 && i==payintentPeriod){
		    			   ...
		    			   if(feePayaccrualType.equals("withRentalFee")){
							   sf2 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectRentalFeeCharging,            // 资金类型
									   DateUtil.addMonthsToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i*3),  //计划日期
										BigDecimal.valueOf(0),                                                // 支出金额
										LeaseFinanceFundIntentListProtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(30)).multiply(new BigDecimal(payintentPeriod)).multiply(new BigDecimal(3)),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
										);
							   /*sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
		    	   ...
		    	   for (int i = 1; i <=payintentPeriod; i++) {
		    		   ...
		    		   if(feePayaccrualType.equals("withRentalFee")){
						   sf2 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectRentalFeeCharging,            // 资金类型
								   intentDate,  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
									LeaseFinanceFundIntentListProtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(dayOfEveryPeriod)),i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						  /* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
								   intentDate,  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
	    		   ...
	    		   if(isPreposePayAccrual == 1){//前置付息
	    			 ...
	    			 if(feePayaccrualType.equals("withRentalFee")){
					   sf2 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectRentalFeeCharging,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								LeaseFinanceFundIntentListProtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(dayOfEveryPeriod)).multiply(new BigDecimal(payintentPeriod)),1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					   /*sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
							   intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
		    		   ...
		    		   if(isPreposePayAccrual ==0 && i==payintentPeriod){//前置付息
		    			   ...
		    			   if(feePayaccrualType.equals("withRentalFee")){
							   sf2 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectRentalFeeCharging,            // 资金类型
									   DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i*dayOfEveryPeriod),  //计划日期
										BigDecimal.valueOf(0),                                                // 支出金额
										LeaseFinanceFundIntentListProtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(dayOfEveryPeriod)).multiply(new BigDecimal(payintentPeriod)),payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
										);
							  /* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
									   DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), i*dayOfEveryPeriod),  //计划日期
										BigDecimal.valueOf(0),                                                // 支出金额                                              // 支出金额
	           ...
	           for (int i = 0; i < payintentPeriod; i++) {
	        	   Date intentDate=null;
					...
					/* }else{
					 ...
					 sf1 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectLeaseFeeCharging,            // 资金类型
							 intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								LeaseFinanceFundIntentListProtw.calculAccrualnew(projectMoney,accrual),i+1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType 	);        // 收入金额
					 if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
					 list.add(sf1);
					 }
					 if(feePayaccrualType.equals("withRentalFee")){
						 sf2 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectRentalFeeCharging,            // 资金类型
						    		intentDate,  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
									LeaseFinanceFundIntentListProtw.calculAccrualnew(projectMoney,managementConsultingOfRate),i+1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
									);
						/* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
									intentDate,  //计划日期
									BigDecimal.valueOf(0),                                                // 支出金额
				...
				if(isPreposePayAccrual==1){
				 ...
				 sf1 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectLeaseFeeCharging,            // 资金类型
						 intentDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							LeaseFinanceFundIntentListProtw.calculAccrualnew(projectMoney,accrual).multiply(new BigDecimal(payintentPeriod)),qs,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType 	);        // 收入金额
				 if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
				 list.add(sf1);
				 }
				 if(feePayaccrualType.equals("withRentalFee")){
					 sf2 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectRentalFeeCharging,            // 资金类型
					    		intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								LeaseFinanceFundIntentListProtw.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(payintentPeriod)),qs,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					/* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
								intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
   		 ...
   		 if(isStartDatePay.equals("1")){ //固定在某日
 			...
 			if(isInterestByOneTime==0){
	 			...
	 			if(isPreposePayAccrual==0){
	 				intentDate=DateUtil.addMonthsToDate(DateUtil.parseDate(date1,"yyyy-MM-dd"), payintentPeriod, -1);
	 				qs=payintentPeriod;
			 ...
			 if(isStartDatePay.equals("1")){ //固定在某日
				...
				// if(payintentPeriod>1){
				 String firstdays=Integer.valueOf(DateUtil.getDaysBetweenDate( date1,sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1)))+1).toString();
				 	sf1 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectLeaseFeeCharging,            // 资金类型
							...
							BigDecimal.valueOf(0),                                                // 支出金额
							LeaseFinanceFundIntentListProtw.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(firstdays)),      // 收入金额
		    	...
		    	/*  BigDecimal financeServiceOfRate1=financeServiceOfRate.multiply(new BigDecimal(firstdays));
				 ...
				 sf1 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectLeaseFeeCharging,            // 资金类型
						 intentDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							LeaseFinanceFundIntentListProtw.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30)),
				 ...
				 sf1 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectLeaseFeeCharging,            // 资金类型
						 intDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							LeaseFinanceFundIntentListProtw.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(enddays)),      // 收入金额
				   ...
				   /* BigDecimal financeServiceOfRate2=financeServiceOfRate.multiply(new BigDecimal(enddays));
					...
					// BigDecimal financeServiceOfRateMoney=new BigDecimal(0);
						 ...
						 String firstdays=Integer.valueOf(DateUtil.getDaysBetweenDate( date1,sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1)))+1).toString();
						 accrualMoney=accrualMoney.add(LeaseFinanceFundIntentListProtw.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(firstdays)));
						 BigDecimal  managementConsultingOfRate1=managementConsultingOfRate.multiply(new BigDecimal(firstdays));
						...
						// financeServiceOfRateMoney=financeServiceOfRateMoney.add(projectMoney.multiply(financeServiceOfRate1));
						 ...
						 for (int i = 2; i <payintentPeriod; i++) {
							 accrualMoney=accrualMoney.add(LeaseFinanceFundIntentListProtw.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30)));
						 ...
						 BigDecimal managementConsultingOfRate2=managementConsultingOfRate.multiply(new BigDecimal(enddays));
						// BigDecimal financeServiceOfRate2=financeServiceOfRate.multiply(new BigDecimal(enddays));
						 accrualMoney=accrualMoney.add(LeaseFinanceFundIntentListProtw.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(enddays)));
		...
		if (accrualType.equals("sameprincipalandInterest") && payaccrualType.equals("monthPay")) {// 月尾
		     BigDecimal periodtimemoney=LeaseFinanceFundIntentListProtw.periodtimemoney(accrual.multiply(new BigDecimal(30)),projectMoney,payintentPeriod).divide(new BigDecimal(1),2,BigDecimal.ROUND_HALF_UP);
		     BigDecimal preperiodsurplus=projectMoney;
		     if(isStartDatePay.equals("1")){ //固定在某日
		    	 if(isInterestByOneTime==0){
			    	 String firstdays=Integer.valueOf(DateUtil.getDaysBetweenDate( date1,sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1)))+1).toString();
			    	 ...
			    	 sf1 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectLeaseFeeCharging,            // 资金类型
			    	  ...
			    	  BigDecimal firstperiodtimemoney=periodtimemoney.add(sf1.getIncomeMoney().subtract(LeaseFinanceFundIntentListProtw.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30))));
					    sf =calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectLeasePrincipalRepayment,            // 资金类型
								DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1),  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
					...
					sf1 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectLeaseFeeCharging,            // 资金类型
							...
							BigDecimal.valueOf(0),                                                // 支出金额
							LeaseFinanceFundIntentListProtw.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30)),      // 收入金额
				    ...
				    sf =calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectLeasePrincipalRepayment,            // 资金类型
							DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), i),  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							periodtimemoney.subtract(LeaseFinanceFundIntentListProtw.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30))),        // 收入金额
							i,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,String.valueOf(payintentPerioDate),accrualType
				    );
				    if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
						list.add(sf1);
					...
					sf1 = calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectLeaseFeeCharging,            // 资金类型
			    	  ...
			    	  BigDecimal endperiodtimemoney=periodtimemoney.subtract((LeaseFinanceFundIntentListProtw.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30))).subtract(sf1.getIncomeMoney()));
					    sf =calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectLeasePrincipalRepayment,            // 资金类型
					    		DateUtil.addDaysToDate(DateUtil.addMonthsToDate(DateUtil.parseDate(date1), payintentPeriod), -1),  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
		    		...
		    		// BigDecimal financeServiceOfRateMoney=new BigDecimal(0);
			    	 ...
			    	 String firstdays=Integer.valueOf(DateUtil.getDaysBetweenDate( date1,sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), 1)))+1).toString();
			    	 accrualMoney=accrualMoney.add(LeaseFinanceFundIntentListProtw.calculAccrualnew(preperiodsurplus1,accrual).multiply(new BigDecimal(firstdays)));
			    	 BigDecimal managementConsultingOfRate1=managementConsultingOfRate.multiply(new BigDecimal(firstdays));
			    	 managementConsultingOfRateMoney=managementConsultingOfRateMoney.add(projectMoney.multiply(managementConsultingOfRate1));
			    	/* BigDecimal financeServiceOfRate1=financeServiceOfRate.multiply(new BigDecimal(firstdays));
			    	 financeServiceOfRateMoney=financeServiceOfRateMoney.add(projectMoney.multiply(financeServiceOfRate1));*/
						...
						preperiodsurplus1=preperiodsurplus1.subtract(periodtimemoney.subtract(LeaseFinanceFundIntentListProtw.calculAccrualnew(preperiodsurplus1,accrual).multiply(new BigDecimal(30))));
						
						
						
					for (int i = 2; i < payintentPeriod; i++) {
						accrualMoney=accrualMoney.add(LeaseFinanceFundIntentListProtw.calculAccrualnew(preperiodsurplus1,accrual).multiply(new BigDecimal(30)));
						managementConsultingOfRateMoney=managementConsultingOfRateMoney.add(projectMoney.multiply(managementConsultingOfRate).multiply(new BigDecimal(30)));
						//financeServiceOfRateMoney=financeServiceOfRateMoney.add(projectMoney.multiply(financeServiceOfRate).multiply(new BigDecimal(30)));
					...
					String enddays=Integer.valueOf(DateUtil.getDaysBetweenDate( sdf.format(DateUtil.addMonthsToDate(DateUtil.parseDate(startDate, "yyyy-MM-dd"), payintentPeriod-1)),sdf.format(DateUtil.addDaysToDate(DateUtil.addMonthsToDate(DateUtil.parseDate(date1), payintentPeriod), -1)))).toString();
					accrualMoney=accrualMoney.add(LeaseFinanceFundIntentListProtw.calculAccrualnew(preperiodsurplus1,accrual).multiply(new BigDecimal(enddays)));
					BigDecimal managementConsultingOfRate2=managementConsultingOfRate.multiply(new BigDecimal(enddays));
					managementConsultingOfRateMoney=managementConsultingOfRateMoney.add(projectMoney.multiply(managementConsultingOfRate2));
					//BigDecimal financeServiceOfRate2=financeServiceOfRate.multiply(new BigDecimal(enddays));
					//financeServiceOfRateMoney=financeServiceOfRateMoney.add(projectMoney.multiply(financeServiceOfRate2));
					...
					if(isPreposePayAccrual==1){
					  ...
					  sf =calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectLeasePrincipalRepayment,            // 资金类型
						...
						preperiodsurplus=preperiodsurplus.subtract(periodtimemoney.subtract(LeaseFinanceFundIntentListProtw.calculAccrualnew(preperiodsurplus,accrual).multiply(new BigDecimal(30))));
						for (int i = 2; i < payintentPeriod; i++) {
					    ...
					    sf =calculslfundintent(LeaseFinanceFundIntentListProtw.ProjectLeasePrincipalRepayment,            // 资金类型
					    		DateUtil.addMonthsToDate(DateUtil.parseDate(date1), payintentPeriod),  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								periodtimemoney.subtract(LeaseFinanceFundIntentListProtw.calculAccrualnew(preperiodsurplus1,accrual).multiply(new BigDecimal(enddays))),        // 收入金额
								payintentPeriod,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,String.valueOf(payintentPerioDate),accrualType
					    );
						list.add(sf);
		...
		if(accrualType.equals("matchingRepayment") && payaccrualType.equals("owerPay")){
			...
			for(int i=0;i<payintentPeriod;i++){
		   			  ...
		   			  sf1 = calculslfundintent(LeaseFinanceFundIntentListPro.ProjectLeaseFeeCharging,            // 资金类型
		   					DateUtil.addDaysToDate(DateUtil.parseDate(date1, "yyyy-MM-dd"), (i+1)*dayOfEveryPeriod-1),  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							accrualMoney,      // 收入金额
							i+1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType
							);
