var updateDroit = function(mortgageid,refreshMortgageGridStore,businessType){
	var anchor = '100%';
				...
				items : [{
					...
					handler : function() {
						...
						if(mortgageName == ""){
							...
							panel_updateDroit.getForm().submit({
								method : 'POST',
								waitTitle : '连接',
								waitMsg : '消息发送中...',
								success : function(form, action) {
									Ext.ux.Toast.msg('操作信息', '保存成功!');
									Ext.getCmp('win_updateDroit').destroy();
		...
		var window_updateDroit = new Ext.Window({
			...
			listeners : {
				'beforeclose' : function(){
					if(panel_updateDroit.getForm().isDirty()){
						Ext.Msg.confirm('操作提示','数据被修改过,是否保存?',function(btn){
							if(btn=='yes'){
								panel_updateDroit.getTopToolbar().items.itemAt(0).handler.call() ;
							}else{
								panel_updateDroit.getForm().reset() ;
								window_updateDroit.destroy() ;
							}
						}) ;
						return false ;
					}
