/**
 * @author: YHZ
 * @class YiKaiConferenceView
 * @extends Ext.Panel
 * @description 已开会议查询
 * @company 北京互融时代软件有限公司
 * @createtime:2010-10-11 PM
 */
YiKaiConferenceView = Ext.extend(Ext.Panel, {
	//搜索
	searchPanel : null,
	// 数据展示Panel
	gridPanel : null,
	...
	constructor : function(_cfg) {
		...
		// 调用父类构造
		YiKaiConferenceView.superclass.constructor.call(this, {
			id : 'YiKaiConferenceView',
			title : '已开会议查询',
			iconCls : 'menu-conference_yikai',
			region : 'center',
			layout : 'border',
	...
	initUIComponents : function() {
		...
		this.store = new Ext.data.JsonStore( {
			url : __ctxPath + "/admin/yiKaiConference.do",
			root : 'result',
		...
		if (btn == 'yes') {
			Ext.Ajax.request( {
				...
				success : function(response, options) {
					...
					Ext.getCmp('YiKaiConferenceGrid').getStore().reload();
				},
 ...
 */
delRecords : function() {
	var gridPanel = Ext.getCmp('YiKaiConferenceGrid');
