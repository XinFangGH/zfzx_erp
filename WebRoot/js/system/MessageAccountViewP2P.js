 ...
 * @company 智维软件
 */
MessageAccountViewP2P = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		MessageAccountViewP2P.superclass.constructor.call(this, {
			id:'MessageAccountViewP2P',
			region : 'center',
			layout : 'border',
				...
				items : [{
					...
					items :[{
						...
						items : [{
							...
							labelSeparator : '',
							anchor : '20%',
							html:'<a href="#" onclick="updateSet()" style="color:#1678E2;padding-left:15px;">修改配置</a>'
						}]
					}]
				}]
			}]
 ...
 */
function updateSet(){
	new UpdateMessInfo({type:'ERP'}).show();
};
...
//	new SMSTest({type:'ERP'}).show();
	...
	Ext.Msg.confirm("提示!", '短信连接测试', function(btn) {
				if (btn == "yes") {
					...
					Ext.Ajax.request({
						url : __ctxPath+ '/system/testP2pMessSystemProperties.do',
						method : 'POST',
