...
Ext.ux.tree.CheckTreeNodeUI = Ext.extend(Ext.tree.TreeNodeUI, {
	...
	,toggleCheck:function() {
		var checked = !this.isChecked();
		//全选 控制
		this.checkNode(this.node,checked);
...
checkNode : function(node, checked) {
			...
			var childHasChecked = function(node, checked) {
				...
				if (childNodes || childNodes.length > 0) {
					for (var i = 0; i < childNodes.length; i++) {
						...
						if (!checkbox)
							continue;
						// 本身是全选中状态 即 checked=true, indeterminate=false ,
						// 并且此次进来取消选中状态(参数checked=false)
			...
			node.cascade(function(n) {// 下
						if (n.ui.checkbox) {
							if (n.getUI().checkbox) {
								
								n.getUI().checkbox.indeterminate = false;
							}
							//n.attributes.checked = n.ui.checkbox.checked = checked;
                            n.ui.setChecked(checked);
							//n.attributes.selectAll = checked;
						}
					}, this);
			node.bubble(function(n) {/*// 不进行向  上 遍历 防止 取消掉子节点 父节点也取消
						if (n != node) {
							var ret = childHasChecked(n, checked);
							if (!checked) { // 取消选中
								
								n.attributes.selectAll = checked;
								if (ret == 1) {
									if (n.getUI().checkbox) {
										n.getUI().checkbox.indeterminate = true;
									}
								} else if (ret == 0) {
									if (n.getUI().checkbox) {
										n.getUI().checkbox.indeterminate = false;
							...
							} else if (n.ui.checkbox) { // 选中
								if (ret == 2) {
									n.attributes.selectAll = true;
									n.getUI().checkbox.indeterminate = false;
								} else if (ret == 1) {
									n.getUI().checkbox.indeterminate = true;
								}
