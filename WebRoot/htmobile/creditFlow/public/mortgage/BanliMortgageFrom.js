...
Ext.define('htmobile.creditFlow.public.mortgage.BanliMortgageFrom', {
    extend: 'Ext.form.Panel',
    
    name: 'BanliMortgageFrom',
		    ...
		    items: [
		            {
		                xtype: 'fieldset',
		                flex:1,
		                title :'<span style="color:#000000;">办理<'+this.mortgagepersontypeforvalue+'>手续</span>',
		                defaults:{
		                	xtype: 'textfield',
		                	width:"100%"
		                //	labelAlign:"top"
		                ...
		                items: [
		                  {
		                    ...
		                    },
		                      {
		                        label: '<div class="fieldlabel">权证编号:</div>',
		                         value:Ext.isEmpty(this.data)?null:this.data.pledgenumber,
		                        name:'procreditMortgage.pledgenumber'
		                    } ,
		                      {
		                        label: '<div class="fieldlabel">登记机关:</div>',
		                         value:Ext.isEmpty(this.data)?null:this.data.enregisterDepartment,
    ...
    addsubmit:function(){
		 ...
		 var transactDate= Ext.getCmp("procreditMortgagetransactDate").getValue();
		  ...
		  if(Ext.isEmpty(transactPerson)){
		   ...
		   var pledgenumber= loginForm.getCmpByName("procreditMortgage.pledgenumber").getValue();
		  if(Ext.isEmpty(pledgenumber)){
		    Ext.Msg.alert('','权证编号不能为空');
			return;
		  }
		   var enregisterDepartment= loginForm.getCmpByName("procreditMortgage.enregisterDepartment").getValue();
		  if(Ext.isEmpty(enregisterDepartment)){
		    Ext.Msg.alert('','登记机关不能为空');
			return;
		  }
