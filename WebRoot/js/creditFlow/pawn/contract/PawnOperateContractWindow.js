 ...
 * @company
 */
PawnOperateContractWindow = Ext.extend(Ext.Window, {
	// 内嵌panelContract
	panelContract : null,
	categoryId:null,
	piKey:null,
	...
	constructor : function(_cfg) {
		...
		PawnOperateContractWindow.superclass.constructor.call(this, {
					layout : 'anchor',
...
//					minWidth : 539,
										...
										listeners : {
											...
											"change" : function(com, checked) {
												...
												if(formPanel.get(3).getName() == "pawnName") {
													formPanel.remove("dw",true);//移除保证人下拉框
												}
												...
												if (inputValue == 1) {
													...
													formPanel.insert(3,pawnColumn);//加入保证人下拉框
													this.setHeight(325);
													this.doLayout();
												}
	...
	autoLoadData : function(id){
		...
		this.panelContract.getForm().load({
			...
			success : function(form, action) {
				...
				if(formPanel.get(3).getName() == "pawnName") {
					formPanel.get(3).setValue(action.result.data.pawnItemName);
				}
	...
	makeContract :function(temId,businessType,piKey,taskId,contractType,categoryId,contractId,a,b,htType,thirdRalationId,isApply,clauseId){/*
		...
		var window = this;
		var remark = this.getCmpByName('remark').getValue();
		if(temId == null){
			...
			}else if(htType =='thirdContract'||htType =='thirdRalationContract'||htType =='ourThirdContract'|| htType=='baozContract' || htType=='otherFiles' || htType=='loanContract' || htType=='pawnContract' || htType=='dwContract' ){
				args = {
					projId : piKey ,
					businessType : businessType,
					templateId : temId,
			...
			if(orMake == 'reMake'){
				Ext.Msg.confirm("提示!", '重新生成将会删除掉您之前上传的合同！是否继续？', function(btn) {
					if (btn == "yes") {
						...
						Ext.Ajax.request({
							url : __ctxPath+'/credit/document/makeContract.do',
							method : 'POST',
							scope : this,
							success : function(response, request) {
								var obj = Ext.util.JSON.decode(response.responseText);
