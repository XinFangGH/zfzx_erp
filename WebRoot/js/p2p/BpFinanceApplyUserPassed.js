 ...
 * @createtime:
 */
BpFinanceApplyUserPassed = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				BpFinanceApplyUserPassed.superclass.constructor.call(this, {
							id : 'BpFinanceApplyUserPassed',
							title : '已通过审核',
							region : 'center',
							layout : 'border',
			...
			initUIComponents : function() {
						...
						items : [
								new Ext.Toolbar.Separator({
									...
									if (rows.length == 0) {
										...
										Ext.Msg.confirm('信息确认', '是否发标', function(btn) {
					       					 if (btn == 'yes') {
												Ext.Ajax.request({
													url:__ctxPath + '/p2p/saveBpFinanceApplyUser.do',
													params:{'loanId':rows[0].data.loanId,'state':10},
													success: function(resp,opts) {
				...
				this.gridPanel=new HT.GridPanel({
					...
					url : __ctxPath + "/p2p/personListBpFinanceApplyUser.do?state=9",
					fields : [{
									name : 'loanId',
									type : 'int'
								}
