...
public class PawnFundIntentListPro {
	/**
	 * 贷款贷出
	 */
	...
	public static List<SlFundIntent> getFundIntentDefaultList(String calcutype,
		...
		if (null ==isHaveLending ||isHaveLending.equals("yes")) { //像提前还款什么的都是不需要生成放款记录
			sf.setFundType(PawnFundIntentListPro.ProjectLoadOut);// 资金类型
		...
		if (accrualType.equals("singleInterest") && payaccrualType.equals("dayPay")) {// 日尾
			if(isInterestByOneTime==0){
	           for (int i = 0; i < payintentPeriod; i++) {
	        	   Date intentDate=null;
					 ...
					 sf1 = calculslfundintent(PawnFundIntentListPro.ProjectLoadAccrual,            // 资金类型
							 intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								PawnFundIntentListPro.calculAccrualnew(projectMoney,accrual),i+1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType 	);        // 收入金额
					 if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
					 list.add(sf1);
					 }
					 sf2 = calculslfundintent(PawnFundIntentListPro.ProjectFinanceService,            // 资金类型
					    		intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
								PawnFundIntentListPro.calculAccrualnew(projectMoney,managementConsultingOfRate),i+1,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
								);
					 /*sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
								intentDate,  //计划日期
								BigDecimal.valueOf(0),                                                // 支出金额
				...
				if(isPreposePayAccrual==1){
				 ...
				 sf1 = calculslfundintent(PawnFundIntentListPro.ProjectLoadAccrual,            // 资金类型
						 intentDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							PawnFundIntentListPro.calculAccrualnew(projectMoney,accrual).multiply(new BigDecimal(payintentPeriod)),qs,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType 	);        // 收入金额
				 if(sf1.getIncomeMoney().compareTo(new BigDecimal(0))!=0){
				 list.add(sf1);
				 }
				 sf2 = calculslfundintent(PawnFundIntentListPro.ProjectFinanceService,            // 资金类型
				    		intentDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
							PawnFundIntentListPro.calculAccrualnew(projectMoney,managementConsultingOfRate).multiply(new BigDecimal(payintentPeriod)),qs,isPreposePayAccrual,payaccrualType,dayOfEveryPeriod,date1,isInterestByOneTime,date2,isStartDatePay,null,accrualType          // 收入金额
							);
				/* sf3 = calculslfundintent(FundIntentListPro3.ProjectFinanceService,            // 资金类型
							intentDate,  //计划日期
							BigDecimal.valueOf(0),                                                // 支出金额
			...
			sf =calculslfundintent(PawnFundIntentListPro.ProjectLoadRoot,            // 资金类型
