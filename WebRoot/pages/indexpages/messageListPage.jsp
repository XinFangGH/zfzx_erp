...
<table class="newsList" cellpadding="0" cellspacing="0" rules="rows">
	<c:forEach var="message" items="${messageList}">
		<tr onMouseOver="this.className='over';" onMouseOut="this.className='out';">
			...
			<td><a href="#"
				onclick="eval(new MessageDetail(${message.receiveId}))"><c:choose><c:when test="${fn:length(message.shortMessage.content)>66}">${fn:substring(message.shortMessage.content,0,66)}...</c:when><c:otherwise>${message.shortMessage.content}</c:otherwise></c:choose></a></td>
