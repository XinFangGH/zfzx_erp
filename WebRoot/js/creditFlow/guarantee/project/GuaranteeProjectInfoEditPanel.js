/**
 * @author lisl
 * @class GuaranteeProjectInfoEditPanel
 * @description 担保项目信息编辑页面
 * @extends Ext.Panel
 */
GuaranteeProjectInfoEditPanel = Ext.extend(Ext.Panel, {
	constructor : function(_cfg) {
		Ext.applyIf(this, _cfg);
		this.initUIComponents();
		GuaranteeProjectInfoEditPanel.superclass.constructor.call(this, {
			id : 'GuaranteeProjectInfoEdit_' + this.record.data.projectId,// id属性值前缀通过是否包含"Edit"来区分查看和编辑页面
	...
	initUIComponents : function() {
		var jsArr = [
				__ctxPath + '/js/creditFlow/guarantee/project/GuaranteeProjectInfoEdit.js'// 项目信息
		];
	...
	constructPanel : function() {
		...
		if (this.projectStatus != 2 && this.projectStatus != 3
			...
			if (this.taskNodeKey != "zmnSbhDraftResolution" && this.taskNodeKey != "glnExaminationArrangement") {// 内部指定处理人员，不容随意更换
				Ext.Ajax.request({
		...
		// 项目信息导航
		this.projectInfoNavigation = new ProjectInfoNavigation({
			record : this.record,
			isActivityComboEdit : this.isActivityComboEdit,
			enabled : this.enabled,
			isCustomerInfoEdit : isGranted('ediCustomer_gt'
					+ this.projectStatus + this.bmStatus),
			flag : "edit" // edit表示编辑页面,see表示查看页面
		});
		// 项目信息
		this.projectInfo = new GuaranteeProjectInfoEdit({
			record : this.record,
