var updateProduct = function(mortgageid,refreshMortgageGridStore,businessType){
			...
			tbar : new Ext.Toolbar({
				...
				handler : function() {
					...
					if(mortgageName == ""){
						...
						panel_updateProduct.getForm().submit({
							method : 'POST',
							waitTitle : '连接',
							waitMsg : '消息发送中...',
							success : function(form, action) {
								Ext.ux.Toast.msg('操作信息', '保存成功!');
								Ext.getCmp('win_updateProduct').destroy();
		...
		var window_updateProduct = new Ext.Window({
			...
			listeners : {
				'beforeclose' : function(){
					if(panel_updateProduct.getForm().isDirty()){
						Ext.Msg.confirm('操作提示','数据被修改过,是否保存?',function(btn){
							if(btn=='yes'){
								panel_updateProduct.getTopToolbar().items.itemAt(0).handler.call() ;
							}else{
								panel_updateProduct.getForm().reset() ;
								window_updateProduct.destroy() ;
							}
						}) ;
						return false ;
					}
