var addMortgageEducation = function(piKey,personTypeParams,assuretypeidParams,refreshMortgageGridStore,businessType){
	var anchor = '100%';
	
	var showEducationWin = function(){
		...
		items : [{
			...
			handler : function() {
				...
				if(mortgageName == ""){
					...
					panel_addEducation.getForm().submit({
						method : 'POST',
						waitTitle : '连接',
						waitMsg : '消息发送中...',
						success : function(form, action) {
							Ext.ux.Toast.msg('操作信息', '保存成功!');
							Ext.getCmp('win_addEducation').destroy();
		...
		var panel_addEducation = new Ext.form.FormPanel({
		id :'addEducation1',
		url : __ctxPath +'/credit/mortgage/education/addEducation.do',
		monitorValid : true,
					...
					items : [{
						...
						onTriggerClick : function(){
							if(businessType=='Financing'){
								selectSlCompanyMain(selectSlCompanyEdu);
							}else{
						...
						onTriggerClick : function(){
							if(businessType=='Financing'){
								selectSlPersonMain(selectSlPersonEdu);
							}else{
	...
	var window_addEducation = new Ext.Window({
		id : 'win_addEducation',
		title :'新增教育用地信息',
		iconCls : 'btn-add',
		...
		modal : true,
		buttonAlign: 'right',
		items : [panel_addEducation],
		listeners : {
			'beforeclose' : function(){
				if(panel_addEducation.getForm().isDirty()){
					Ext.Msg.confirm('操作提示','是否保存当前新添加的数据?',function(btn){
						if(btn=='yes'){
							panel_addEducation.getTopToolbar().items.itemAt(0).handler.call() ;
						}else{
							panel_addEducation.getForm().reset() ;
							window_addEducation.destroy() ;
						}
					}) ;
					return false ;
				}
