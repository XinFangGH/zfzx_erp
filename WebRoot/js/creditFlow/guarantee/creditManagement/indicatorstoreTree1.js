//定量的tree
function setCatalogMenuFun1(node,e,centerPanel){
		var setCatalogMenu; 
		...
		if((nodeId == null)){ //当前节点为根节点
			...
			setCatalogMenu = new Ext.menu.Menu({
				items :[{
					...
					handler : function (){
						updateCatalogType1(node);
					}
				},"-",{
					text : '删除定量评估要素',
					hidden : isGranted('_yaosu_delete')?false:true,
					handler : function (){
						deleteCatalogType1(node,centerPanel);
					}
				}]
			})
	...
	//添加目录，添加指标
	function addCatalogType1(isLaef,node){
		...
		var menu_Panel = new Ext.Panel({
			...
			items : [{
				...
				labelWidth: 80,
				url :__ctxPath+'/creditFlow/indicator/addIndicatorStore.do?indicatorStore.parentId='+node.id+ '&indicatorStore.isleaf='+isLaef,
				waitMsgTarget: true,
			    monitorValid:true,
			    labelAlign: 'right',
			    buttonAlign : 'center',
	...
	function updateCatalogType1(node){
		var id = node.id;
		var text = node.text;
	...
	function deleteCatalogType1(node,centerPanel){
		var id = node.id;
