 ...
 * @createtime:
 */
LeaseFundIntentPrincipalPayOwe = Ext.extend(Ext.Panel, {
			// 构造函数
			constructor : function(_cfg) {
				...
				// 调用父类构造
				LeaseFundIntentPrincipalPayOwe.superclass.constructor.call(this, {
							id : 'LeaseFundIntentPrincipalPayOwe'+this.businessType,
			...
			initUIComponents : function() {
				...
				this.topbar = new Ext.Toolbar({
					items : [{
						...
						xtype : 'button',
						scope : this,
						hidden : isGranted('_liushui_f_principalPay_'+this.businessType+'_'+this.type)?false:true,
...
//					}
					,{
						...
						xtype : 'button',
						scope : this,
						hidden : isGranted('_liushuisee_f_principalPay_'+this.businessType+'_'+this.type)?false:true,
						handler : this.openliushuiwin1
			
					},new Ext.Toolbar.Separator({
						hidden : isGranted('_liushuisee_fprincipalPay_'+this.businessType+'_'+this.type)?false:true
					}),{
						iconCls : 'btn-ok',
						text :'核销',
						xtype : 'button',
						scope : this,
						hidden : isGranted('_ping_f_principalPay_'+this.businessType+'_'+this.type)?false:true,
						...
						xtype : 'button',
						scope : this,
						hidden : isGranted('_uploaddownpz_f_principalPay_'+this.businessType+'_'+this.type)?false:true,
						handler : this.upload
			        	
			
					},new Ext.Toolbar.Separator({
						hidden : (isGranted('_uploaddownpz_f_principalPay_'+this.businessType+'_'+this.type)?false:true)||(isGranted('_previewpz_f_'+this.businessType+'_'+this.type)?false:true)
					}),{
						iconCls : 'btn-setting',
						text :'预览凭证',
						xtype : 'button',
						scope : this,
						hidden : isGranted('_previewpz_f_principalPay_'+this.businessType+'_'+this.type)?false:true,
...
//							}
							, {
									header : this.type=='rentalCostsPaid'?'放款金额':'计划支出金额',
								dataIndex : 'payMoney',
								align :'right',
								width : 150,
								renderer:function(v){
								...
								header : this.type=='rentalCostsPaid'?'计划放款日':'计划支出日期',
								width : 100,
								dataIndex : 'intentDate',
								align : 'center'
						//		sortable:true
							}, {
								header : this.type=='rentalCostsPaid'?'实际放款日':'实际支出日期',
								width : 100,
								dataIndex : 'factDate'
						//		sortable:true
							}, {
				...
				if(this.businessType!=null){
					...
					switch (businessType) {
					  ...
					  case "Pawn": sqlstr="&fundType=('pawnPrincipalLending')";
					    break;
					 case "LeaseFinance": (this.type=='rentalCostsPaid')?sqlstr="&fundType=('rentalCostsPaid')":sqlstr="&fundType=('marginRefund')";
					    break;
