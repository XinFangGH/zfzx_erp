...
GlContractArchiveView = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		GlContractArchiveView.superclass.constructor.call(this, {
//					title : '合同确认',
					region : 'center',
					layout : 'anchor',
					items : [this.glContractArchive_gridPanel]
				});
	},// end of constructor
	// 初始化组件
	initUIComponents : function() {
		...
		this.glContractArchive_gridPanel = new HT.EditorGridPanel({
			plugins : [checkColumn],
			hiddenCm :  true,
			border : false,
			isShowTbar:false,
			//name : 'glContractArchive_gridPanel',
			scope : this,
			store : jStore,
			autoScroll : true,
			autoWidth : true,
			...
			columns : [
				{
					...
					renderer : function(val, m, r) {
						if(r.get('contractId')==''||r.get('contractId')==0){
							...
							return '<a style ="TEXT-DECORATION:underline;cursor:pointer" onclick="GlContractArchiveView.downloadContract(\''+r.get('contractId')+'\')" >下载</a>';//个人贷款合同
						}
					}
				}, {
					header : '附件下载',
					...
					sortable : true,
					renderer : function(val,m,r){
						return  '<a style ="TEXT-DECORATION:underline;cursor:pointer" onclick="GlContractArchiveView.DownFiles(\''+r.get('id')+'\',\''+this.projId+'\',\''+r.get('contractCategoryText')+'\',\''+r.get('contractId')+'\',\''+r.get('mortgageId')+'\')">'+r.get('fileCount')+'</a>';
					}
				},checkColumn, {
					header : '备注',
			...
			listeners : {
				scope : this,
				//glContractArchive_gridPanel数据发生改变时请求后台方法修改数据并重新加载显示数据
...
GlContractArchiveView.DownFiles = function(id,piKey,contractName,contractId,mortgageId){
	var mark = "cs_procredit_contract."+contractId;
	uploadfile('下载附件',mark,0,null,null,null);
};
/*GlContractArchiveView.lookContractByHTML = function(id){
	var pbar = Ext.MessageBox.wait('数据读取中','请等待',{
		interval:200,
    	increment:15
	});
	Ext.Ajax.request({
		url : __ctxPath+'/creditcontract/ajaxIsExistValida.do',
		method : 'POST',
		success : function(response, request){
...
GlContractArchiveView.downloadContract = function(conId){
...
//查看要素
/*GlContractArchiveView.lookElement = function(title,tempId,piKey,conId,cType){
	var pbar = Ext.MessageBox.wait('数据读取中','请等待',{
		interval:200,
    	increment:15
	});
	...
	Ext.Ajax.request({
		url : __ctxPath+'/credit/document/validateExist.do',
		method : 'POST',
		success : function(response, request){
			var elementCodeStore = new Ext.data.JsonStore( {
				url : __ctxPath+'/credit/document/findElementCall.do',
				root : 'topics',
			...
			var elementWin = new Ext.Window({
				...
				tbar : [new Ext.Button({text:'下载查看合同',tooltip : '下载并预览合同文本',
					scope : this,
					handler : function(){
						GlContractArchiveView.downloadContract(conId);
					}}
				)]
			});
		},
