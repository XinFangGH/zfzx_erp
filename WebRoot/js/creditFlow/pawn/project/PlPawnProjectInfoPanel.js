 ...
 * @extends Ext.Panel
 */
PlPawnProjectInfoPanel = Ext.extend(Ext.Panel, {
	constructor : function(_cfg) {
		Ext.applyIf(this, _cfg);
		this.initUIComponents();
		PlPawnProjectInfoPanel.superclass.constructor.call(this, {
			id : 'PlPawnProjectInfo_' + this.record.data.projectId,// id属性值前缀,通过是否包含"Edit"来区分查看和编辑页面
			title : "项目查看-" + this.record.data.projectName,
			iconCls : 'btn-edit',
			border : false,
			autoHeight : true,
			items : []
	...
	constructPanel : function() {
		...
		// 项目信息
		this.projectInfo = new PlPawnProjectInfo({
			record : this.record
		});
