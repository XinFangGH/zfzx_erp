package com.zhiwei.credit.dao.creditFlow.contract.impl;
/*
 *  北京互融时代软件有限公司   -- http://www.hurongtime.com
 *	Copyright @ 2004 - 2010 Yuseen.com all rights reserved.京ICP备 05007290 号
*/
import com.zhiwei.core.dao.impl.BaseDaoImpl;
import com.zhiwei.credit.dao.creditFlow.contract.CsElementCategoryDao;
import com.zhiwei.credit.model.creditFlow.contract.CsElementCategory;

/**
 * 
 * @author 
 *
 */
@SuppressWarnings("unchecked")
public class CsElementCategoryDaoImpl extends BaseDaoImpl<CsElementCategory> implements CsElementCategoryDao{

	public CsElementCategoryDaoImpl() {
		super(CsElementCategory.class);
	}

}