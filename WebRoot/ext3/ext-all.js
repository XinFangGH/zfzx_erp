/*
 * Ext JS Library 3.2.0 Copyright(c) 2006-2010 Ext JS, Inc. licensing@extjs.com
	...
	p = {
		...
		insertHtml : function(z, t, A) {
			...
			if (t.insertAdjacentHTML) {
				if (l.test(t.tagName)
						&& (u = d(t.tagName.toLowerCase(), z, t, A))) {
					return u
				}
...
Ext.Element.addMethods(function() {
	var h = {}, y = /(-[a-z])/gi, b = {}, s = document.defaultView, u = Ext.isIE
			? "styleFloat"
			: "cssFloat", D = /alpha\(opacity=(.*)\)/i, l = /^\s+|\s+$/g, B = Ext.Element, d = "padding", c = "margin", z = "border", t = "-left", q = "-right", x = "-top", o = "-bottom", j = "-width", r = Math, A = "hidden", e = "isClipped", k = "overflow", n = "overflow-x", m = "overflow-y", C = "originalClip", i = {
		l : z + t + j,
		r : z + q + j,
		t : z + x + j,
		b : z + o + j
	...
	return {
		...
		getStyle : function() {
			return s && s.getComputedStyle ? function(L) {
				...
				G = (F = I.style[L]) ? F : (H = s.getComputedStyle(I, ""))
						? H[L]
						: null;
				if (K) {
					if (G == "rgba(0, 0, 0, 0)") {
...
(function() {
	var z = null, B = undefined, k = true, t = false, j = "setX", h = "setY", a = "setXY", n = "left", l = "bottom", s = "top", m = "right", q = "height", g = "width", i = "points", x = "hidden", A = "absolute", u = "visible", e = "motion", o = "position", r = "easeOut", d = new Ext.Element.Flyweight(), v = {}, y = function(
			C) {
		return C || {}
	}, p = function(C) {
		d.dom = C;
	...
	Ext.Fx = {
		...
		slideIn : function(I, F) {
			...
			K.queueFx(F, function() {
				...
				J = p(H).switchStatements(I.toLowerCase(), R, {
							...
							r : [E, N, M.width, M.height, j, M.right, n, s, z,
									z, O],
							b : [E, N, M.width, M.height, h, M.bottom, n, s, z,
									G, O],
							tl : [E, N, 0, 0, z, z, m, l, L, G, O],
							bl : [E, N, 0, 0, h, M.y + M.height, m, s, L, G, O],
							br : [E, N, 0, 0, a, [M.right, M.bottom], n, s, L,
									G, O],
		...
		highlight : function(E, I) {
			...
			G.queueFx(I, function() {
						...
						arguments.callee.anim = p(H).fxanim(D, I, "color", 1,
								"easeIn", J)
					});
			return G
		},
		frame : function(C, F, I) {
			...
			E.queueFx(I, function() {
						...
						var M = p(H).getXY(), K = {
							...
							D = p(document.body || document.documentElement)
									.createChild({
												style : {
													position : A,
													"z-index" : 35000,
		...
		shift : function(E) {
			...
			this.queueFx(E, function() {
						...
						if (C.x || C.y || C.xy) {
							C.points = C.xy || {
								to : [C.x ? C.x.to : p(D).getX(),
										C.y ? C.y.to : p(D).getY()]
							}
						}
						arguments.callee.anim = p(D).fxanim(C, E, e, 0.35, r,
								function() {
									p(D).afterFx(E)
								})
					});
		...
		ghost : function(F, D) {
			...
			H.queueFx(D, function() {
						...
						L.by = p(E).switchStatements(F.toLowerCase(),
								function(O, N) {
									return [O, N]
								}, {
									t : [0, -G],
	...
	Ext.extend(Ext.data.Connection, Ext.util.Observable, {
		...
		doFormUpload : function(q, j, k) {
			...
			function s() {
				...
				try {
					B = r.contentWindow.document || r.contentDocument
							|| g.frames[l].document;
					if (B) {
						if (B.body) {
							if (/textarea/i
...
Ext.XTemplate = function() {
	...
	i = ["<tpl>", i, "</tpl>"].join("");
	while ((q = i.match(p))) {
		var b = q[0].match(d), a = q[0].match(t), z = q[0].match(v), g = null, h = null, r = null, y = b
				&& b[1] ? b[1] : "";
		if (a) {
			g = a && a[1] ? a[1] : null;
			if (g) {
...
Ext.layout.VBoxLayout = Ext.extend(Ext.layout.BoxLayout, {
	...
	calculateChildBoxes : function(x, e) {
		var k = x.length, u = this.padding, z = u.top, p = u.left, q = z
				+ u.bottom, a = p + u.right, y = e.width - this.scrollOffset, v = e.height, B = Math
				.max(0, y - a), l = this.pack == "start", m = this.pack == "center", n = this.pack == "end", J = 0, F = 0, A = 0, j = [], h, d, g, b, t, D, C, s, G, H, E;
		for (C = 0; C < k; C++) {
			h = x[C];
			g = h.height;
			d = h.width;
		...
		for (C = 0; C < k; C++) {
			...
			switch (this.align) {
				case "stretch" :
					E = B - H;
					s.width = E.constrain(h.minHeight || 0, h.maxWidth
									|| 1000000);
					s.dirtySize = true;
					break;
				case "stretchmax" :
...
Ext.layout.HBoxLayout = Ext.extend(Ext.layout.BoxLayout, {
	...
	calculateChildBoxes : function(A, e) {
		var n = A.length, y = this.padding, D = y.top, r = y.left, s = D
				+ y.bottom, a = r + y.right, C = e.width - this.scrollOffset, z = e.height, h = Math
				.max(0, z - s), o = this.pack == "start", p = this.pack == "center", q = this.pack == "end", J = 0, B = 0, E = 0, m = [], l, d, g, c, x, G, F, v, I, j, t;
		for (F = 0; F < n; F++) {
			l = A[F];
			g = l.height;
			d = l.width;
...
Ext.slider.MultiSlider = Ext.extend(Ext.BoxComponent, {
	...
	setValue : function(d, c, b, g) {
		...
		if (c !== a.value
				&& this.fireEvent("beforechange", this, c, a.value) !== false) {
			a.value = c;
...
Ext.data.ArrayReader = Ext.extend(Ext.data.JsonReader, {
	readRecords : function(r) {
		...
		for (var z = 0, B = u.length; z < B; z++) {
			var t = u[z], a = {}, p = ((d || d === 0) && t[d] !== undefined
					&& t[d] !== "" ? t[d] : null);
...
Ext.MessageBox = function() {
	...
	return {
		...
		updateText : function(B) {
			...
			l.update(B || "&#160;");
			var z = d != "" ? (j.getWidth() + j.getMargins("lr")) : 0, D = l
					.getWidth()
					+ l.getMargins("lr"), A = u.getFrameWidth("lr"), C = u.body
					.getFrameWidth("lr"), y;
			if (Ext.isIE && z > 0) {
				z += 3
			}
...
var swfobject = function() {
	var F = "undefined", s = "object", U = "Shockwave Flash", Y = "ShockwaveFlash.ShockwaveFlash", r = "application/x-shockwave-flash", T = "SWFObjectExprInst", z = "onreadystatechange", Q = window, k = document, u = navigator, V = false, W = [i], p = [], P = [], K = [], m, S, G, D, L = false, a = false, o, I, n = true, O = function() {
		var ac = typeof k.getElementById != F
				&& typeof k.getElementsByTagName != F
				&& typeof k.createElement != F, aj = u.userAgent.toLowerCase(), aa = u.platform
		...
		if (typeof u.plugins != F && typeof u.plugins[U] == s) {
			...
			if (ad
					&& !(typeof u.mimeTypes != F && u.mimeTypes[r] && !u.mimeTypes[r].enabledPlugin)) {
				V = true;
				Z = false;
		...
		if ((typeof k.readyState != F && k.readyState == "complete")
				|| (typeof k.readyState == F && (k.getElementsByTagName("body")[0] || k.body))) {
			g()
		}
		if (!L) {
	...
	function R(ac, ad, Z, ab) {
		...
		if (ag) {
			...
			var af = O.ie && O.win ? "ActiveX" : "PlugIn", ae = "MMredirectURL="
					...
					+ "&MMplayerType=" + af + "&MMdoctitle=" + k.title;
	...
	function x(ae, aa, af, ad) {
		...
		if (!o || I != Z) {
			...
			o = ac.appendChild(ab);
			if (O.ie && O.win && typeof k.styleSheets != F
...
Ext.form.FieldSet = Ext.extend(Ext.Panel, {
    ...
    onRender : function(ct, position){
        ...
        //coupled with workFlow Form  add by gao 
	   	if(this.title!=null&&this.ownerCt&&this.ownerCt.ownerCt&&this.ownerCt.ownerCt.ownerCt&&this.ownerCt.ownerCt.ownerCt.ownerCt&&this.ownerCt.ownerCt.ownerCt.ownerCt.flagInFlowFlag){//ProcessNextForm中的flagInFlowFlag标识  
	   		this.commenthistory=typeof(this.commenthistory)=="undefined"?true:this.commenthistory;//如果配置 指定false 则必然false
	   		this.projectId = this.projectId||this.ownerCt.ownerCt.projectId;
	   		this.businessType=this.businessType||this.ownerCt.ownerCt.businessType;
	   		//在HTExt.js override时用到
	   		//this.componentKey=this.componentKey||title;//如果componentKey没指定，默认使用title，强烈推荐title做key
	   	}
        if(this.checkboxToggle||this.commenthistory){//gaomarker
            var o = this.commenthistory===true?{tag: 'input', type: 'button',style:'height:16px;width:16px;border:0px;background:url('+__ctxPath +'/images/zhiwei_new.gif);', name: this.checkboxName || this.id+'-checkbox'}:(typeof this.checkboxToggle == 'object' ?
                    this.checkboxToggle :
                    {tag: 'input', type: 'checkbox', name: this.checkboxName || this.id+'-checkbox'});
...
Ext.grid.GridView = Ext.extend(Ext.util.Observable, {
	...
	doRender : function(d, v, m, a, r, t) {
		var e = this.templates, c = e.cell, z = e.row, o = r - 1;
...
Ext.grid.GridView.SplitDragZone = Ext.extend(Ext.dd.DDProxy, {
			constructor : function(a, b) {
				...
				Ext.grid.GridView.SplitDragZone.superclass.constructor.call(
						this, b, "gridSplitters" + this.grid.getGridEl().id, {
			...
			handleMouseDown : function(g) {
				...
				if (c && this.allowHeaderDrag(g)) {
					var d = this.view.fly(c).getXY(), a = d[0], h = d[1];
...
Ext.grid.GroupingView = Ext.extend(Ext.grid.GridView, {
	...
	doRender : function(c, h, q, a, p, s) {
		...
		var A = this.getGroupField(), o = this.cm.findColumnIndex(A), x, j = "width:"
