 ...
 * @author Jiang Wanyu
 */
 var picViewer = function(mark,readOnly,typeisfile,projectId,businessType){
 	//alert(33333333);
 	//alert(typeisfile);
    ...
    if(typeof(readOnly)!="undefined"){
 	...
 	var url=basepath();
 	url=url+"creditFlow/fileUploads/getFileListExtFileForm.do";
 	if(typeisfile !=null){
 	...
 	if(!isReadOnly){
			...
			'<tpl for=".">',
			    ...
			    '</div>',
			    '<span onclick="download1({[this.download(values.fileid)]})" style="text-decoration:underline;cursor:pointer;height:15px"></span><span  onclick="download(values.fileid)"  if="this.imgOrDir(type) == false" class="x-editable" >{filename}</span></div>',//下载
	        '</tpl>',
		 ...
		 returnUrl:function(extendname,webpath){
		  ...
		  download:function(v){
			...
			'<tpl for=".">',
			    ...
			    '</div>',
			    '<span onclick="download1({[this.download(values.fileid)]})" style="text-decoration:underline;cursor:pointer;height:15px"></span><span   if="this.imgOrDir(type) == false" class="">{filename}</span></div>', //下载
	        '</tpl>',
	        '<div class="x-clear"></div>'
		,{
		 returnUrl:function(extendname,webpath){
            		...
            		//  
            			var i=0;
			               ...
			               //img+=record.get('webPath')+',';
			               		img[i]=/*p2pUrl*/basepath()+"/"+record.get('webPath');
			                	i++;
			               });
