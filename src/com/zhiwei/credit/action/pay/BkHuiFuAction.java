...
IsUnFreeze + UnFreezeOrdId + FreezeTrxId + BgRetUrl + MerPriv + RespExt ;
	...
	private void sign(String msgData) {
		...
		if (isSuccess) {
			...
			if (CmdId.equals("UserRegister")) {
				...
				//updateAccount("1","2");
				updateAccount(ObAccountDealInfo.T_RECHARGE,ObAccountDealInfo.DEAL_STATUS_2,ObAccountDealInfo.DIRECTION_INCOME);
			}else if(CmdId.equals("InitiativeTender")||CmdId.equals("AutoTender")){
				//updateAccount("4","1");
				updateAccount(ObAccountDealInfo.T_INVEST,ObAccountDealInfo.DEAL_STATUS_7,ObAccountDealInfo.DIRECTION_PAY);
			}else if(CmdId.equals("Loans")){
				//updateAccount("4","2");
				updateAccount(ObAccountDealInfo.T_INVEST,ObAccountDealInfo.DEAL_STATUS_2,ObAccountDealInfo.DIRECTION_PAY);
			}
			message = RespDesc;
		} else {
			...
			if (CmdId.equals("NetSave")) {
				//updateAccount("1","3");
				updateAccount(ObAccountDealInfo.T_RECHARGE,ObAccountDealInfo.DEAL_STATUS_3,ObAccountDealInfo.DIRECTION_INCOME);
			}else if(CmdId.equals("InitiativeTender")||CmdId.equals("AutoTender")){
				//updateAccount("4","1");
				updateAccount(ObAccountDealInfo.T_INVEST,ObAccountDealInfo.DEAL_STATUS_1,ObAccountDealInfo.DIRECTION_PAY);
			}else if(CmdId.equals("Loans")){
				//updateAccount("4","3");
				updateAccount(ObAccountDealInfo.T_INVEST,ObAccountDealInfo.DEAL_STATUS_3,ObAccountDealInfo.DIRECTION_PAY);
			}
		}
	 ...
	 *            2 成功 3 失败
	 */
	private void updateAccount(String type ,Short state,Short direction) {
		bpCustMember = getMemberByRet(UsrCustId);
		...
		map.put("dealDirection",direction);//交易方向:1收入，2支出（参见ObAccountDealInfo中的常量）
